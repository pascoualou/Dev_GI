/*------------------------------------------------------------------------
File        : cinech_CRUD.p
Purpose     : Librairie contenant les procedures liées à la mise à jour de la table cinech
...
define variable ghttcinech as handle no-undo.      // le handle de la temp table à mettre à jour
...
procedure getCinech:
    ...
    Notes  : service externe. Critère pcMat-num = ? si pas à prendre en compte
    ------------------------------------------------------------------------------*/
    define input parameter piSoc-cd      as integer    no-undo.
    define input parameter piEtab-cd     as integer    no-undo.
    define input parameter piType-invest as integer    no-undo.
