...
procedure setAfamqtord:
    ...
    ------------------------------------------------------------------------------*/
    define input parameter table-handle phttAfamqtord.
    ghttAfamqtord = phttAfamqtord.
    run crudAfamqtord.
    delete object phttAfamqtord.
end procedure.
...
blocTrans:
    do transaction:
        repeat:
            ...
            create afamqtord.
            if not outils:copyValidField(buffer afamqtord:handle, vhttBuffer, "C", mtoken:cUser)
            then undo blocTrans, leave blocTrans.
        end.
    end.
    vhttquery:query-close().
