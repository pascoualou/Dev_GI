...
derniere revue: 2018/04/13 - phm: KO
             enlever les messages pour le déploiment
             revoir le champ iden1 pour le passer en format "9999999999" et éviter int(...) = pi...
                for each tbdet
                   where tbdet.cdent = pcCodeEntete
                     and integer(tbdet.iden1) = piIdentifiant1:
...
procedure deleteTbdetSurIdentifiant1:
    /*------------------------------------------------------------------------------
    Purpose: suppression de tous les enregistrements correspondant aux parametres de selection 
    Notes  : service externe
    ------------------------------------------------------------------------------*/
    define input parameter pcCodeEntete   as character no-undo.
    define input parameter piIdentifiant1 as int64     no-undo.
    
    define buffer tbdet for tbdet.

message "deleteTbdetSurIdentifiant1 " pcCodeEntete "// " piIdentifiant1. 

blocTrans:
    do transaction:
        for each tbdet exclusive-lock
           where tbdet.cdent        = pcCodeEntete
             and int64(tbdet.iden1) = piIdentifiant1:
            delete tbdet no-error.
            if error-status:error then do:
                mError:createError({&error}, error-status:get-message(1)).
                undo blocTrans, leave blocTrans.
