...
message "deleteSalarSurRole " pcTypeRole "// " piNumeroRole.

blocTrans:
    do transaction:
        for each salar exclusive-lock
           where salar.tprol = pcTypeRole
             and salar.norol = piNumeroRole:  
            delete salar no-error.
            if error-status:error then do:
                mError:createError({&error}, error-status:get-message(1)).
                undo blocTrans, leave blocTrans.
