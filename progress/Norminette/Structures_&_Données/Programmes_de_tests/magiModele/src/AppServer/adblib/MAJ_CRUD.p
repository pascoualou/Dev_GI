...
procedure deleteMajSurDocument:
    /*------------------------------------------------------------------------------
    Purpose: suppression de tous les enregistrements correspondant aux parametres de selection 
    Notes  : service externe
    ------------------------------------------------------------------------------*/
    define input parameter pcBase     as character no-undo.
    define input parameter pisociete  as integer   no-undo.
    define input parameter pcTable    as character no-undo.
    define input parameter piDocument as integer   no-undo.
    
    define buffer maj for maj.

message "deleteMajSurDocument " pcBase "// " pisociete "// " pcTable "// " piDocument.

blocTrans:
    do transaction:
        for first maj exclusive-lock     
            where maj.nmlog          = pcBase
              and maj.soc-cd         = pisociete
              and maj.nmtab          = pcTable
              and integer(maj.cdenr) = piDocument:
            delete maj no-error.    
            if error-status:error then do:
                mError:createError({&error}, error-status:get-message(1)).
                undo blocTrans, leave blocTrans.
