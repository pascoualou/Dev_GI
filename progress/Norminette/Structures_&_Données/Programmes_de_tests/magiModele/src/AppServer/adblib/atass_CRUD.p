/*------------------------------------------------------------------------
File        : atass_CRUD.p
Purpose     : Librairie contenant les procedures liées à la mise à jour de la table atass
...
define variable ghttatass as handle no-undo.     // le handle de la temp table à mettre à jour
...
procedure readAtass:
    /*------------------------------------------------------------------------------
    Purpose: Lecture d'un enregistrement de la table atass Table pour l'Ã©dition de l'attestation ASSEDIC
    Notes  : service externe
    ------------------------------------------------------------------------------*/
...
procedure getAtass:
    /*------------------------------------------------------------------------------
    Purpose: Lecture des enregistrements de la table atass Table pour l'Ã©dition de l'attestation ASSEDIC
...
procedure deleteAtassSurMandatRole:
    /*------------------------------------------------------------------------------
...
message "deleteAtassSurMandatRole " piNumeroMandat "// " piNumeroRole.

blocTrans:
    do transaction:
        for each atass no-lock
           where atass.nomdt = piNumeroMandat
             and atass.norol = piNumeroRole:  
            find current atass exclusive-lock.      
            delete atass no-error.
            if error-status:error then do:
                mError:createError({&error}, error-status:get-message(1)).
                undo blocTrans, leave blocTrans.
