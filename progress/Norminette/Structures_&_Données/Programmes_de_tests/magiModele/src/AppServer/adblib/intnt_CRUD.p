...
procedure deleteIntntSurContrat:
    /*------------------------------------------------------------------------------
...
message "deleteIntntSurContrat "  pcTypeContrat "// " piNumeroContrat.
...
procedure deleteIntntSurIdentifiant:
    /*------------------------------------------------------------------------------
    Purpose: suppression de tous les enregistrements correspondant aux parametres de selection 
    Notes  : service externe
    ------------------------------------------------------------------------------*/
...
message "deleteIntntSurIdentifiant "  pcTypeContrat "// " piNumeroContrat.

blocTrans:
    do transaction:
        for each intnt exclusive-lock
           where intnt.tpidt = pcTypeContrat
...
procedure deleteRoleMandant:
    /*------------------------------------------------------------------------------
    Purpose: suppression intnt et enregistrement dependant pour la suppression d'un role mandant 
    Notes  : service externe
    ------------------------------------------------------------------------------*/
...
message "deleteRoleMandant "  piNumeroRole.
           ...
           where intnt.tpidt = pcTypeRole
            ...
            then do:
                for each acpte exclusive-lock 
                   where acpte.tpcon = intnt.tpcon
                     and acpte.nocon = intnt.nocon
                     and acpte.norol = piNumeroRole:
                ...
                for each aecha exclusive-lock 
                   where aecha.soc-cd          = integer(mToken:cRefPrincipale)
                     and aecha.etab-cd         = intnt.nocon
                     and integer(aecha.cpt-cd) = piNumeroRole:
                ...
                for each ahono no-lock  
                   where ahono.nomdt = intnt.nocon:
                    find current ahono exclusive-lock.    
            ...
            if intnt.tpcon = {&TYPECONTRAT-blocNote}
            then do:
...
procedure deleteContratBlocNote:
    /*------------------------------------------------------------------------------
    Purpose: suppression de tous les enregistrements correspondant aux parametres de selection 
    Notes  : service externe
    ------------------------------------------------------------------------------*/
...
message "deleteContratBlocNote "  pcTypeIdentifiant "// " piNumeroIdentifiant.
