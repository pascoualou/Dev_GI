...
procedure deleteCumdasSurRole:
    /*------------------------------------------------------------------------------
    Purpose: suppression de tous les enregistrements correspondant aux parametres de selection 
    Notes  : service externe
    ------------------------------------------------------------------------------*/
    define input parameter pcTypeRole   as character no-undo.
    define input parameter pdNumeroRole as decimal   no-undo.
    
    define buffer cumdas for cumdas.

message "deleteCumdasSurRole " pcTypeRole "// " pdNumeroRole.

blocTrans:
    do transaction:
        for each cumdas exclusive-lock
           where cumdas.tprol = pcTypeRole
             and cumdas.norol = pdNumeroRole:  
            delete cumdas no-error.
            if error-status:error then do:
                mError:createError({&error}, error-status:get-message(1)).
                undo blocTrans, leave blocTrans.
