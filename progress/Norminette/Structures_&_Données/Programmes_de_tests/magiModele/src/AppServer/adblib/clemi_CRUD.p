...
procedure readClemi:
    ...
    for first clemi no-lock
        where clemi.tpcon = pcTpcon
          ...
          and clemi.cdcle = pcCdcle : // index ix-clemi04   
...
procedure getClemi:
    ...
    then for each clemi no-lock 
        where clemi.noimm = piNoimm
          ...
          and clemi.cdcle = (if pcCdcle <> ? and pcCdcle > "" then pcCdcle else clemi.cdcle)
        ...
        where clemi.tpcon = pcTpcon
          and clemi.nocon = piNocon   
          and clemi.cdcle = (if pcCdcle <> ? and pcCdcle > "" then pcCdcle else clemi.cdcle)
          and clemi.cdeta = (if pcStatut = "TOUT" then clemi.cdeta else "V") : // index ix-clemi04 
        vhttBuffer:buffer-create().
        outils:copyValidField(buffer clemi:handle, vhttBuffer).  // copy table physique vers temp-table
    end.
    delete object phttClemi no-error.
...
procedure deleteClemitEtLienSurImmeuble:
    ...
    Notes  : service externe 
    ------------------------------------------------------------------------------*/
    define input parameter piNumeroCle      as integer   no-undo.
...
message "deleteClemitEtLienSurImmeuble "  piNumeroCle.

blocTrans:
    do transaction:
        for each clemi exclusive-lock 
           where clemi.noimm = piNumeroCle:
            if clemi.cdcle < "A" 
            then do:
