...
Purpose     : Librairie contenant les procedures liÈes ‡ la mise ‡ jour de la table suimandatsepa
...
Notes       : permet de travailler sur un sous ensemble de colonnes de la table ‡ condition
              que les champs de l'index unique soient tous prÈsents.
------------------------------------------------------------------------*/

{oerealm/include/instanciateTokenOnModel.i}      // Doit Ítre positionnÈe juste aprËs using
define variable ghttsuimandatsepa as handle no-undo.     // le handle de la temp table ‡ mettre ‡ jour
...
function getIndexField returns logical private(phBuffer as handle, output phNomprelsepa as handle, output phNolig as handle):
    /*------------------------------------------------------------------------------
    Purpose: rÈcupËre les handles des n champs de l'index unique
...
procedure readSuimandatsepa:
    /*------------------------------------------------------------------------------
    Purpose: Lecture d'un enregistrement de la table suimandatsepa Suivi des pr√©l√®vements et modifications des mandats de pr√©l√®vement SEPA
    Notes  : service externe
    ------------------------------------------------------------------------------*/
...
procedure getSuimandatsepa:
    /*------------------------------------------------------------------------------
    Purpose: Lecture des enregistrements de la table suimandatsepa Suivi des pr√©l√®vements et modifications des mandats de pr√©l√®vement SEPA
    Notes  : service externe. CritËre piNomprelsepa = ? si pas ‡ prendre en compte
    ------------------------------------------------------------------------------*/
    define input parameter piNomprelsepa as int64  no-undo.
    define input parameter table-handle phttSuimandatsepa.
