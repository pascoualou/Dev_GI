...
procedure deleteAhon2SurMandat:
    /*------------------------------------------------------------------------------
...
message "deleteAhon2SurMandat "  piNumeroMandat.

blocTrans:
    do transaction:
        for each ahon2 no-lock
           where ahon2.nomdt = piNumeroMandat:
            find current ahon2 exclusive-lock.   
            delete ahon2 no-error.
            if error-status:error then do:
                mError:createError({&error}, error-status:get-message(1)).
                undo blocTrans, leave blocTrans.
