...
procedure getListeNumeroContratDispo:
    ...
    assign
        ...
        voFournisseurLoyer = new parametrageFournisseurLoyer("00001") /* Dans tous les cas on recherche le paramètre fournisseurs loyer, Valeurs par défaut (RESIDE ETUDES) */
        voSyspg            = new syspg()
...
boucle:
    for each aprof no-lock
       where aprof.profil-cd = if pcTypeContrat = {&TYPECONTRAT-mandat2Gerance} then 21 else 91:
        ...
        else if lookup(vcCdModele, "00003,00004") > 0    /* modif SY le 04/01/2010 */
...
boucle2:
        do viNoCttEnc = viNoDebCtt to viNoFinCtt:
            ...
            or (lookup(vcCdModele, "00003,00004") > 0
        ...
        /* Ajout SY le 04/01/2010 */
        if lookup(vcCdModele, "00003,00004") > 0 and lookup(pcNatureContrat, vcNatureLocSsLoc) = 0
