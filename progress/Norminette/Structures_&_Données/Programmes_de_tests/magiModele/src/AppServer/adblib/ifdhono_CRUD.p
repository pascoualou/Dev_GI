...
procedure deleteIfdhonoSurEtablissement:
    /*------------------------------------------------------------------------------
...
message "deleteIfdhonoSurEtablissement "  piSociete "// " piEtablissement.

blocTrans:
    do transaction:
        for each ifdhono exclusive-lock
           where ifdhono.soc-cd  = piSociete
             and ifdhono.etab-cd = piEtablissement:
            delete ifdhono no-error.
            if error-status:error then do:
                mError:createError({&error}, error-status:get-message(1)).
                undo blocTrans, leave blocTrans.
