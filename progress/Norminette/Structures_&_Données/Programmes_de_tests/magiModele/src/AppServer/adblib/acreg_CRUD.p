...
procedure readAcreg:
    /*------------------------------------------------------------------------------
    Purpose: Lecture d'un enregistrement de la table acreg Accords de r̬glement
    Notes  : service externe
    ------------------------------------------------------------------------------*/
...
procedure getAcreg:
    /*------------------------------------------------------------------------------
    Purpose: Lecture des enregistrements de la table acreg Accords de r̬glement
...
procedure deleteAcregSurContrat:
    /*------------------------------------------------------------------------------
...
message "deleteAcregSurContrat "  pcTypeContrat "// " piNumeroContrat.

blocTrans:
    do transaction:
        for each acreg exclusive-lock   
           where acreg.tpcon = pcTypeContrat
...
procedure deleteAcregSurMandat:
    /*------------------------------------------------------------------------------
    Purpose: suppression de tous les enregistrements correspondant aux parametres de selection 
    Notes  : service externe
    ------------------------------------------------------------------------------*/
...
message "deleteAcregSurMandat "  pcTypeMandat "// " piNumeroMandat.

blocTrans:
    do transaction:
        for each acreg exclusive-lock
           where acreg.tpmdt = pcTypeMandat
             and acreg.nomdt = piNumeroMandat:
            delete acreg no-error.
            if error-status:error then do:
                mError:createError({&error}, error-status:get-message(1)).
                undo blocTrans, leave blocTrans.
