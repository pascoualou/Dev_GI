...
procedure readDifheu:
    /*------------------------------------------------------------------------------
    Purpose: Lecture d'un enregistrement de la table difheu Salari̩s : DIF - cr̩dit d'heures
    Notes  : service externe
    ------------------------------------------------------------------------------*/
...
procedure getDifheu:
    /*------------------------------------------------------------------------------
    Purpose: Lecture des enregistrements de la table difheu Salari̩s : DIF - cr̩dit d'heures
...
message "deleteDifheuSurRole " pcTypeRole "// " piNumeroRole.

blocTrans:
    do transaction:
        for each difheu exclusive-lock
           where difheu.tprol = pcTypeRole
             and difheu.norol = piNumeroRole:  
            delete difheu no-error.
            if error-status:error then do:
                mError:createError({&error}, error-status:get-message(1)).
                undo blocTrans, leave blocTrans.
