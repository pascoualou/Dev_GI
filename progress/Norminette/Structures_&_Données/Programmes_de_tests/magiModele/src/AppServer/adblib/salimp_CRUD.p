...
procedure readSalimp:
    /*------------------------------------------------------------------------------
    Purpose: Lecture d'un enregistrement de la table salimp Imputation comptable par salari̩,cl̩, rub et sous-rub ana
    Notes  : service externe
    ------------------------------------------------------------------------------*/
...
procedure getSalimp:
    /*------------------------------------------------------------------------------
    Purpose: Lecture des enregistrements de la table salimp Imputation comptable par salari̩,cl̩, rub et sous-rub ana
...
message "deleteSalimpSurRole " pcTypeRole "// " piNumeroRole.

blocTrans:
    do transaction:
        for each salimp exclusive-lock
           where salimp.tprol = pcTypeRole
             and salimp.norol = piNumeroRole:  
            delete salimp no-error.
            if error-status:error then do:
                mError:createError({&error}, error-status:get-message(1)).
                undo blocTrans, leave blocTrans.
