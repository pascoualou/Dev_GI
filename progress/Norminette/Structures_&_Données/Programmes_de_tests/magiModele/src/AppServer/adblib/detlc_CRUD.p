...
procedure deleteDetlcSurContrat:
    /*------------------------------------------------------------------------------
...
message "deleteDetlcSurContrat "  pcTypeContrat "// " piNumeroContrat.

blocTrans:
    do transaction:
        for each detlc exclusive-lock   
            where detlc.tpcon = pcTypeContrat
              and detlc.nocon = piNumeroContrat:
            delete detlc no-error.
            if error-status:error then do:
                mError:createError({&error}, error-status:get-message(1)).
                undo blocTrans, leave blocTrans.
