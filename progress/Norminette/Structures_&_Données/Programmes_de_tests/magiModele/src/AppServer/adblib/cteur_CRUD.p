...
procedure readCteur:
    ...
    for first cteur no-lock
        where cteur.noimm = piNoimm         // TODO : à remplacer par tpcon/nocon quand les structures de cteur seront corrigées (V19.00 ?)
...
procedure getCteur:
    ...
    ------------------------------------------------------------------------------*/
    define input parameter piNoimm as int64     no-undo.
    define input parameter piNolot as integer   no-undo.
    define input parameter pcTpcpt as character no-undo.
    define input parameter table-handle phttCteur.
    define variable vhttBuffer as handle  no-undo.
...
procedure deleteEnMasseCteur:
    ...
    then viNumeroImmeuble = 10000 + piNumeroContrat.
    ...
    for each cteur exclusive-lock
        where cteur.noimm = viNumeroImmeuble       // TODO à remplacer par tpcon/nocon quand les structures de cteur seront corrigées (V19.00 ?)
          and cteur.TpCpt = pcTypeCompteur:
        delete cteur.
    end.
