...
{adblib/include/aecha.i}
{adblib/include/acpte.i}
{adblib/include/etabl.i}
{role/include/roles.i}
{adresse/include/ladrs.i}
{tiers/include/ttDbTiers.i}
...
procedure ctrlAvantMaj private:
    ...
    for each ttRoleContractant where lookup(ttRoleContractant.CRUD, "C,D") > 0:
        if not voSyspg:isParamExist(gcTypeParametreRole, gcCleParametreRole, ttRoleContractant.cTypeRole)
        then do:
            mError:createError({&error}, 1000607, ttRoleContractant.cTypeRole).   //type de rôle &1 non autorisé
            return.
        end.
        if ttRoleContractant.cTypeRole <> gcRolePrincipal
        then do:
...
procedure initComboContractant:
    ...
    voSyspg = new syspg().
    if ctrat.tpcon = {&TYPECONTRAT-preBail}
    then voSyspg:creationComboSysPgZonXX("R_CLR", "CMBROLECONTRACTANT", "L", ctrat.tpcon, output table ttCombo by-reference).
    else voSyspg:creationComboSysPgZonXX("R_CR1", "CMBROLECONTRACTANT", "L", ctrat.ntcon, output table ttCombo by-reference).
    ...
    then do:
        ...
        for each sys_pg no-lock
           where sys_pg.tppar = gcTypeParametreRole
            ...
            if sys_pg.zone2 = {&TYPEROLE-syndicat2copro}
            then do:
            ...
            if vlNumeroParDefaut
            and can-find(first vbroles no-lock                                         
...
procedure chgContractant private:
    ...
    for first vbRoles no-lock
            ...
            if ctrat.ntcon = {&NATURECONTRAT-specialVacant} then do:
                ...
                for first vb2roles no-lock
                    ...
                    assign
                        ...
                        ttRoles.cdmsy       = mtoken:cUser + "|CHPTC"
                    .
                end.
                /* changer nom */
                create ttCtrat.
                assign
                    ...
                    ttCtrat.lbnom = vcLibelleIntervenant
                    ttCtrat.lnom2 = vcLibelleNomTiers
                    ttCtrat.cdmsy = mtoken:cUser + "|CHPTC"
                    ttCtrat.CRUD        = "U"
                    ttCtrat.rRowid      = rowid(ctrat)
                    ttCtrat.dtTimestamp = datetime(ctrat.dtmsy, ctrat.hemsy)
                .
                ...
                for first ladrs no-lock
                    where ladrs.tpidt = gcRolePrincipal
                      and ladrs.noidt = piNouvNumeroRole
                      and ladrs.tpadr = {&TYPEADRESSE-Principale}
                  , first vbladrs no-lock
                    where vbladrs.tpidt = ctrat.tprol
                      and vbladrs.noidt = ctrat.norol
                      and vbladrs.tpadr = {&TYPEADRESSE-Principale}:
                    create ttLadrs.
                    assign
                        ttLadrs.tpidt = vbladrs.tpidt
                        ttLadrs.noidt = vbladrs.noidt
                        ttLadrs.tpadr = vbladrs.tpadr
                        ttLadrs.dtdeb = vbladrs.dtdeb
                        ttLadrs.tpfrt = ladrs.tpfrt
                        ttLadrs.novoi = ladrs.novoi      /* No de Voie.     */
                        ttLadrs.cdadr = ladrs.cdadr      /* Code bis/ter    */
                        ttLadrs.noadr = ladrs.noadr      /* No d'Adresse.   */
                        ttLadrs.cdte1 = ladrs.cdte1      /* Code Tel. No 1. */
                        ttLadrs.note1 = ladrs.note1      /* No de Tel No 1. */
                        ttLadrs.cdte2 = ladrs.cdte2      /* Code Tel. No 2. */
                        ttLadrs.note2 = ladrs.note2      /* No de Tel No 2. */
                        ttLadrs.cdte3 = ladrs.cdte3      /* Code Tel. No 3. */
                        ttLadrs.note3 = ladrs.note3      /* No de Tel No 3. */
                        ttLadrs.lbdiv = ladrs.lbdiv
                        ttLadrs.cdmsy = mtoken:cUser + "|CHPTC"
                        ttLadrs.CRUD        = "U"
                        ttLadrs.rRowid      = rowid(vbladrs)
                        ttLadrs.dtTimestamp = datetime(vbladrs.dtmsy, vbladrs.hemsy)
                    .
                    ...
                    run dupliqueTelephones in vhProcFcttelep(gcRolePrincipal, piNouvNumeroRole, vbladrs.tpidt, vbladrs.noidt).
                end.
            end.
        end.
        ...
        for first tiers no-lock
            ...
            assign
                ...
                ttTiers.cdmsy       = mtoken:cUser + "@gespcc00.p"
                ttTiers.CRUD        = "U"
            ...
            for each etabl no-lock
                ...
                assign
                    ...
                    ttEtabl.cdmsy       = mtoken:cUser + "@gespcc00.p"
                    ttEtabl.CRUD        = "U"
    ...
    lancementPgm("compta/ietab_CRUD.p",        "setIetab", table ttIetab by-reference).
    lancementPgm("adblib/aecha_CRUD.p",        "setAecha", table ttAecha by-reference).
    lancementPgm("adblib/acpte_CRUD.p",        "setAcpte", table ttAcpte by-reference).
    lancementPgm("tache/tache.p",              "setTache", table ttTache by-reference).
    lancementPgm("adblib/etabl_CRUD.p",        "setEtabl", table ttEtabl by-reference).
    lancementPgm("role/roles_CRUD.p",          "setRoles", table ttRoles by-reference).
    lancementPgm("adresse/ladrs_CRUD.p",       "setLadrs", table ttLadrs by-reference).
    lancementPgm("tiers/tiers_CRUD.p",         "setTiers", table ttTiers by-reference).
