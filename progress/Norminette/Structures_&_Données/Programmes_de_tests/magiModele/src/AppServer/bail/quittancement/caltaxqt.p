...
Notes       : reprise de adb/src/quit/caltaxqt.p
...
derniere revue: 2018/09/13 - phm: KO
...
15 16/06/1998  SY    Initialisation gcCodeRetour à "00" sinon on considère que l'absence de calcul des taxes est une erreur.
...
define variable gcNatureBail                  as character no-undo.
...
define temp-table ttRubriqueSoumisTva no-undo
    ...
    index primaire is primary unique noLoc noRub noLib
.
...
function crettRubriqueSoumisTva returns logical(phttrub as handle):
    ...
    if lookup(string(phttrub::iNorubrique, "999") + string(phttrub::iNoLibelleRubrique, "99"), gcRubriqueFiscaliteSpecifique) = 0
    then do:
        /* Stockage Rubriques pour application tva Bail */
        create ttRubriqueSoumisTva.
        assign
            ttRubriqueSoumisTva.noLoc     = giNumeroBail
            ttRubriqueSoumisTva.norub     = phttrub::iNorubrique
            ttRubriqueSoumisTva.nolib     = phttrub::iNoLibelleRubrique
            ttRubriqueSoumisTva.norub-TVA = giNumeroRubriqueTaxe
            ttRubriqueSoumisTva.nolib-TVA = giLibelleRubriqueTaxe
            ttRubriqueSoumisTva.vlmtq-TVA = round(phttrub::dMontantQuittance * gdeValeurTaux / 100 , 2)   /* total proraté */
            ttRubriqueSoumisTva.mttot-TVA = round(phttrub::dMontantTotal * gdeValeurTaux / 100 , 2)   /* total periode */
        .
    end.
end function.

procedure lancementCaltaxqt:
    /*------------------------------------------------------------------------
    Purpose :
    ...
    assign
        ...
        gcNatureBail          = poCollectionContrat:getCharacter("cNatureContrat")
...
procedure caltaxqtPrivate private:
    ...
    for each detail no-lock
        where detail.cddet = gcTypeBail
          and detail.nodet = giNumeroBail
...
boucleCttac1:
    ...
    for each cttac  no-lock
        ...
        if goSyspg:zone8 = "T@Taxe|" then do:
            ...
            for each ttRub
              ...
              , first bxrbp no-lock
                where bxrbp.ntbai = gcNatureBail
                  and bxrbp.prg05 = cttac.tptac
                  and Bxrbp.NoRub = ttRub.iNorubrique:
                assign
                    gdeMontantQuittanceRubrique = gdeMontantQuittanceRubrique + ttRub.dMontantQuittance
    ...
    for first ttQtt
        ...
        assign
            ...
            viMoisTraitement    = ttQtt.iMoisTraitementQuitt
        .
    end.
    assign
    ...
    for each ttRub
        where ttRub.iNumeroLocataire = giNumeroBail
          and ttRub.iNoQuittance = giNumeroQuittance
          and ttRub.iFamille = 04:
        create ttRubriqueServiceAnnexe.
        assign
            ttRubriqueServiceAnnexe.noLoc = giNumeroBail
            ttRubriqueServiceAnnexe.norub = ttRub.iNorubrique
            ttRubriqueServiceAnnexe.nolib = ttRub.iNoLibelleRubrique
            ttRubriqueServiceAnnexe.vlmtq = ttRub.dMontantQuittance    /* total proraté */
            ttRubriqueServiceAnnexe.mttot = ttRub.dMontantTotal    /* total periode */
            ttRubriqueServiceAnnexe.CdTva = vcTableauTva[ttRub.iSousFamille]
            ttRubriqueServiceAnnexe.rbTva = viTableauRubrique[ttRub.iSousFamille]
            ttRubriqueServiceAnnexe.lbTva = viTableauLibelle[ttRub.iSousFamille]
        .
    end.
        ...
        if goSyspg:zone8 = "T@Taxe|" then do:
            ...
            if available tache then do:
                ...
                for first ttRub
                    where ttRub.iNumeroLocataire = giNumeroBail
                      and ttRub.iNoQuittance = giNumeroQuittance
                      and ttRub.iNorubrique = giNumeroRubriqueTaxe
                      and ttRub.iNoLibelleRubrique = giLibelleRubriqueTaxe:
                    delete ttRub.
                end.
                    ...
                    when {&TYPETACHE-TVABail} then do:
                        ...
                        if gdeValeurTaux <> 0 then do:
                            case vcModeCalcul:
                                ...
                                when {&MODECALCUL-quittanceMoinsCharges} then run calTotQtt-Chg(output vdeTaxe006, output vdeTotal006).       /* Total quittance - charges */
                            end case.
                            /* Création rubrique de TVA du Bail */
                            viNumeroRubriqueTvaBail = 0.
                            for each ttRubriqueSoumisTva where ttRubriqueSoumisTva.noloc = giNumeroBail:
                                assign
                ...
                if vcCodeQuotePart > "" then do:
                    ...
                    if goSyspr:zone1 <> 0 then do:
                        ...
                        if vcTypeParametre = "CDTAD" then do:
                            if not isTaxCRL(integer(truncate(giNumeroBail / 100000, 0)), viMoisQuittancement, vdeLoyerDroitBail, vdeLoyerMinimum) then next boucle2Cttac.
                        end.
                        assign
...
procedure creRubTax private:
    ...
    for first ttRubqt:
        ...
        assign
            ...
            ttRub.iNorubrique = giNumeroRubriqueTaxe
            ttRub.iNoLibelleRubrique = giLibelleRubriqueTaxe
            ...
            ttRub.dMontantQuittance = gdeTaxeQuittance
...
procedure calTotCommun private:
    /*------------------------------------------------------------------------
    Purpose :
    Notes   :
    ------------------------------------------------------------------------*/
    ...
    for each vbttRub
      ...
      , first rubqt no-lock
        where rubqt.cdrub = vbttRub.iNorubrique
          and rubqt.cdlib = vbttRub.iNoLibelleRubrique:
        /* SY 1011/0158 : réécriture du code et mise en include */
        if f_isRubSoumiseTVABail(pcModeCalcul, rubqt.cdfam, rubqt.cdsfa, rubqt.cdrub, integer(rubqt.prg05))
        then do:
            crettRubriqueSoumisTva(input buffer vbttRub:handle).
            assign
                pdeMontantQuittance = pdeMontantQuittance + vbttRub.dMontantQuittance
                pdeMontantTotal     = pdeMontantTotal     + vbttRub.dMontantTotal
            .
        end.
    end.
...
procedure calTotQtt private:
    ...
    run calTotCommun ({&MODECALCUL-quittance}, output pdeMontantQuittance, output pdeMontantTotal).
...
procedure calTotQtt-chg private:
    ...
    run calTotCommun ({&MODECALCUL-loyerEtChargesEtTaxes}, output pdeMontantQuittance, output pdeMontantTotal).
...
procedure calTotImp private:
    ...
    run calTotCommun ({&MODECALCUL-quittanceMoinsCharges}, output pdeMontantQuittance, output pdeMontantTotal).
...
procedure calLoyTax private:
    ...
    run calTotCommun ({&MODECALCUL-loyerEtTaxes}, output pdeMontantQuittance, output pdeMontantTotal).
...
procedure calTotLoy private:
    ...
    for each vbttRub
        where vbttRub.iNumeroLocataire = giNumeroBail
          and vbttRub.iNoQuittance = giNumeroQuittance
          and vbttRub.iFamille = 01
          and vbttRub.iSousFamille <> 04:
        crettRubriqueSoumisTva(input buffer vbttRub:handle).
        assign
            pdeMontantQuittance = pdeMontantQuittance + vbttRub.dMontantQuittance
            pdeMontantTotal     = pdeMontantTotal     + vbttRub.dMontantTotal
...
procedure calTotLch private:
    ...
    for each vbttRub
        where vbttRub.iNumeroLocataire = giNumeroBail
          and vbttRub.iNoQuittance = giNumeroQuittance
          and ((vbttRub.iFamille = 01 and vbttRub.iSousFamille <> 04) or vbttRub.iFamille = 02):
        crettRubriqueSoumisTva(input buffer vbttRub:handle).
        assign
            pdeMontantQuittance = pdeMontantQuittance + vbttRub.dMontantQuittance
            pdeMontantTotal     = pdeMontantTotal     + vbttRub.dMontantTotal
...
boucle:
    for each detail no-lock
      ...
      , first vbttRub
        where vbttRub.iNumeroLocataire = giNumeroBail
          and vbttRub.iNoQuittance = giNumeroQuittance
          and vbttRub.iNorubrique = integer(substring(detail.ixd01, 1, 3, "character"))
          and vbttRub.iNoLibelleRubrique = integer(substring(detail.ixd01, 4, 2, "character")):
        /* ignorer rubrique faisant déjà partie de l'extraction */
        find first ttRubriqueSoumisTva
            where ttRubriqueSoumisTva.noLoc = giNumeroBail
              and ttRubriqueSoumisTva.norub = vbttRub.iNorubrique
              and ttRubriqueSoumisTva.nolib = vbttRub.iNoLibelleRubrique no-error.
