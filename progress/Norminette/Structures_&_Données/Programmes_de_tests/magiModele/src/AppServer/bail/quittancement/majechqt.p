...
define variable giNumeroContrat         as integer   no-undo.
...
function determinationPeriodicite returns integer():
    ...
    Notes :
    -------------------------------------------------------------------------*/
    case ttQtt.cPeriodiciteQuittancement:
        when {&PERIODICITEQUITTANCE-mensuel}            then return 12.     /* Mesuelle */
        when {&PERIODICITEQUITTANCE-bimestrielJanFev} 
     or when {&PERIODICITEQUITTANCE-bimestrielFevMar}   then return  6.     /* Bimensuelle */
        when {&PERIODICITEQUITTANCE-trimestrielJanMar} 
     or when {&PERIODICITEQUITTANCE-trimestrielFevAvr} 
     or when {&PERIODICITEQUITTANCE-trimestrielMarMai}  then return  4.     /* Trimestrielle */
        when {&PERIODICITEQUITTANCE-semestrielJanJun} 
     or when {&PERIODICITEQUITTANCE-semestrielFevJui} 
     or when {&PERIODICITEQUITTANCE-semestrielMarAou}
     or when {&PERIODICITEQUITTANCE-semestrielAvrSep} 
     or when {&PERIODICITEQUITTANCE-semestrielMaiOct} 
     or when {&PERIODICITEQUITTANCE-semestrielJunNov}   then return  2.     /* Semestrielle */
        when {&PERIODICITEQUITTANCE-AnnuelJanDec} 
     or when {&PERIODICITEQUITTANCE-AnnuelFevJan} 
     or when {&PERIODICITEQUITTANCE-AnnuelMarFev}
     or when {&PERIODICITEQUITTANCE-AnnuelAvrMar} 
     or when {&PERIODICITEQUITTANCE-AnnuelMaiAvr} 
     or when {&PERIODICITEQUITTANCE-AnnuelJunMai}
     or when {&PERIODICITEQUITTANCE-AnnuelJuiJun} 
     or when {&PERIODICITEQUITTANCE-AnnuelAouJui} 
     or when {&PERIODICITEQUITTANCE-AnnuelSepAou}
     or when {&PERIODICITEQUITTANCE-AnnuelOctSep} 
     or when {&PERIODICITEQUITTANCE-AnnuelNovOct} 
     or when {&PERIODICITEQUITTANCE-AnnuelDecNov}       then return  1.     /* Annuelle */
    end case.
    return 0.
end function.
    ...
    for each chaff no-lock
        ...
        for each tache no-lock
            where tache.tpcon = gcTypeContrat
              ...
              and tache.notac < ttQtt.iMoisTraitementQuitt
...
procedure majechqtPrivate private:
    ...
    if not available tache or tache.cdhon <> "00002" then return.
...
boucleQuittance:
    for each ttQtt
        ...
        /* Sortie du locataire */
        if vdaFin <> ? and vdaFin < ttQtt.daFinQuittancement then leave boucleQuittance.

boucleEchlo:
        for each echlo no-lock
            where echlo.tpcon = gcTypeContrat
              and echlo.nocon = giNumeroContrat
              and echlo.dtdeb <= ttQtt.daDebutQuittancement
              and (echlo.dtfin > ttQtt.daDebutQuittancement or echlo.dtfin = ?)
        ...
        for each echlo no-lock
            ...
            if (echlo.dtfin = ? or echlo.dtfin >= ttQtt.daFinQuittancement)
            ...
            else do:    /* quittancement sur plusieurs echelles mobiles */
                assign
...
procedure calculLoyerVariable private:
    /*-------------------------------------------------------------------------
...
boucle:
    do viBoucle = 1 to 40:
        ...
        for each chaff no-lock
            ...
            if chaff.cacoc <> 0 and chaff.dtcom <= ttQtt.daFinQuittancement and chaff.dtcom >= ttQtt.daDebutQuittancement
            then do:
                assign
                ...
                .
                /* pénalité de retard */
                if chaff.dtcom <= ttQtt.daFinQuittancement and chaff.dtcom >= ttQtt.daDebutQuittancement
            ...
            assign
                glRegulVersement  = (chaff.cavec <> 0.00 and chaff.dtver <= ttQtt.daFinQuittancement and chaff.dtver >= ttQtt.daDebutQuittancement)
...
procedure loyPlsEch private:
    ...
    assign
        viNombreJourAnnee = interval(date(01, 01, year(ttQtt.daDebutQuittancement) + 1), date(01, 01, year(ttQtt.daDebutQuittancement)), "days")
    ...
    for each echlo no-lock
        where echlo.tpcon = gcTypeContrat
          ...
          and echlo.dtdeb <= ttQtt.daFinQuittancement
    ...
    for each echlo no-lock
        ...
        assign
            ...
            giDureeProrata  = absolute(if echlo.dtfin = ? or echlo.dtfin > ttQtt.daFinQuittancement
                                       then (ttQtt.daFinQuittancement - echlo.dtdeb)
                                       else (echlo.dtfin - if echlo.dtdeb <= ttQtt.daDebutQuittancement
                                                           then ttQtt.daDebutQuittancement else echlo.dtdeb)) + 1
    ...
    if available echlo then do:
        ...
        /* pénalité de retard */
        if glPenaliteRetard
        then gdeSommePenalite = gdeSommePenalite + (gdePenaliteRetard * gdeLoyerMensuel / 100).
        assign
            ...
            gdeLoyerCalcule   = round(gdeLoyerCalcule * viNombreJourAnnee / giMoisPeriode / (absolute(ttQtt.daFinQuittancement - ttQtt.daDebutQuittancement) + 1), 2)
            gdeLoyerCalcule   = gdeLoyerCalcule + gdeRegularisation + gdeSommePenalite
        .
        for first tache exclusive-lock
            where tache.tpcon = gcTypeContrat
...
procedure majttQtt private:
    ...
    find first ttRub
        where ttRub.iNumeroLocataire = ttQtt.iNumeroLocataire
          ...
          and ttRub.iNorubrique = 101 no-error.
    if available ttRub then do:
        ...
        then do:   /* La rubrique 101 existe et le montant calcule est nul, suppression de la rubrique 101 */
            ...
            then for last tache no-lock
                ...
                assign
                    ttRub.iProrata = 1
                    vdeMontant    = ((gdeLoyerCalcule - integer(tache.pdges) / 100 - integer(tache.tpges) / 100) * ttRub.iDenominateurProrata)
                                / ttRub.iNumerateurProrata  + integer(tache.pdges) / 100 + integer(tache.tpges) / 100
                    vdeMontant    = round(vdeMontant, 2)
                .
            end.
            assign
                ttRub.iNoLibelleRubrique = giFamilleRubrique
                ttRub.daDebutApplication = ttQtt.daDebutPeriode
                ttRub.daFinApplication = ttQtt.daFinPeriode
                ttRub.dMontantQuittance = vdeMontant
                ttRub.dMontantTotal = gdeLoyerCalcule
            .
        end.
    end.
    else do:
    /* La rubrique 101 n'existe pas et le montant calcule est non nul --> creation de la rubrique 101 en 1ere position */
        ...
        if gdeLoyerCalcule <> 0 then do:
            ...
            assign
                ...
                ttRub.dMontantQuittance = round(((gdeLoyerCalcule * ttQtt.iNumerateurProrata) / ttQtt.iDenominateurProrata),2)
                ttRub.daDebutApplication = ttQtt.daDebutPeriode
                ttRub.daFinApplication = ttQtt.daFinPeriode
    ...
    for each ttRub where ttRub.iNumeroLocataire = ttQtt.iNumeroLocataire
        and ttRub.iNoQuittance = ttQtt.iNoQuittance:
        gdeLoyerCalcule = gdeLoyerCalcule + ttRub.dMontantQuittance.
    end.
    assign
        ttQtt.dMontantQuittance      = gdeLoyerCalcule
        ttQtt.cdmaj      = 1
        gdeLoyerCalcule  = 0
        gdeLoyerVariable = 0
    .
...
procedure maJTache private:
    ...
    for last chaff no-lock
        where chaff.tpcon = gcTypeContrat
          and chaff.nocon = giNumeroContrat:
        run readTache in ghProcTache(gcTypeContrat, giNumeroContrat, {&TYPETACHE-EchelleMobileLoyer}, ttQtt.iMoisTraitementQuitt, table ttTache by-reference).
        ...
        then ttTache.crud  = "U".
            ...
            assign
                ...
                ttTache.notac = ttQtt.iMoisTraitementQuitt  // on prend le mois de traitement
            .
        end.
        assign
            ttTache.dtdeb = {&dateNulle}
            ttTache.dtfin = {&dateNulle}
            ttTache.tpfin = string(chaff.noper) + separ[2] + string(chaff.nocal)
            ttTache.duree = 0
            ttTache.dtree = {&dateNulle}
            ttTache.ntges = ""
            ttTache.tpges = ""
            ttTache.pdges = ""
            ttTache.cdreg = ""
