...
procedure getEqprov:
    ...
    define input parameter piMsqpv as integer no-undo.
    define input parameter table-handle phttEqprov.
    ...
    vhttBuffer = phttEqprov:default-buffer-handle.
    if piMsqpv = ?
    then for each eqprov no-lock
        where eqprov.noloc = piNoloc
          and eqprov.noqtt = piNoqtt:
        vhttBuffer:buffer-create().
        outils:copyValidField(buffer eqprov:handle, vhttBuffer).  // copy table physique vers temp-table
    end.
    else for each eqprov no-lock
        where eqprov.noloc = piNoloc
          and eqprov.noqtt = piNoqtt
          and eqprov.msqpv = piMsqpv:
        vhttBuffer:buffer-create().
        outils:copyValidField(buffer eqprov:handle, vhttBuffer).  // copy table physique vers temp-table
    end.
    delete object phttEqprov no-error.
