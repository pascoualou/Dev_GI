...
procedure deleteSyngeSurContrat1:
    /*------------------------------------------------------------------------------
    Purpose: suppression de tous les enregistrements correspondant aux parametres de selection 
    Notes  : service externe
    ------------------------------------------------------------------------------*/
    define input parameter pcTypeContrat1   as character no-undo.
    define input parameter piNumeroContrat1 as integer   no-undo.
...
blocTrans:
    do transaction:
        for each synge exclusive-lock
           where synge.tpct1 = pcTypeContrat1
             and synge.noct1 = piNumeroContrat1:
            delete synge no-error.
            if error-status:error then do:
                mError:createError({&error}, error-status:get-message(1)).
                undo blocTrans, leave blocTrans.
...
procedure deleteSyngeSurContrat2:
    /*------------------------------------------------------------------------------
    Purpose: suppression de tous les enregistrements correspondant aux parametres de selection 
    Notes  : service externe
    ------------------------------------------------------------------------------*/
    define input parameter pcTypeContrat2   as character no-undo.
    define input parameter piNumeroContrat2 as integer   no-undo.

    define buffer synge for synge.

blocTrans:
    do transaction:
// whole-index corrige par la creation dans la version d'un index sur tpct2 noct2     
        for each synge exclusive-lock
           where synge.tpct2 = pcTypeContrat2
             and synge.noct2 = piNumeroContrat2:
            delete synge no-error.
            if error-status:error then do:
                mError:createError({&error}, error-status:get-message(1)).
                undo blocTrans, leave blocTrans.
