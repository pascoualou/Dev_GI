...
derniere revue: 2018/09/13 - phm: KO
        todo  pourquoi sur create ou update un traitement supplémentaire, et pas sur delete ?
------------------------------------------------------------------------*/
...
blocTrans:
    do transaction:
        repeat:
            ...
            if vcType2Before <> {&TYPECONTRAT-RIB} 
            and rlctt.tpct2 = {&TYPECONTRAT-RIB}    then tracageContratBanque(rlctt.tpct1, rlctt.noct1).
...
blocTrans:
    do transaction:
        repeat:
            ...
            if rlctt.tpct2 = {&TYPECONTRAT-RIB}    then tracageContratBanque(rlctt.tpct1, rlctt.noct1).
...
blocTrans:
    do transaction:
        repeat:
            ...
            vcType2 = if vhTpct2:buffer-value() > "" then vhTpct2:buffer-value() else {&TYPECONTRAT-RIB}.
            find first rlctt exclusive-lock
...
// todo    sur create, update, traitement particulier. Pourquoi pas sur delete ?
...
procedure creationCompteBancaire:
    ...
    Notes  : service externe contractantContrat.p
             repris de adb/comm/incliadb.i   procedure CreCparib.
    todo  a mettre dans arib_crud ???!!! 
    todo gga procedure a retester 
    ------------------------------------------------------------------------------*/
    ...
    or pcTypeContratMaitre = {&TYPECONTRAT-Salarie}
    ...
    assign
        viNumeroContrat = if pcTypeContratMaitre = {&TYPECONTRAT-titre2copro} or pcTypeContratMaitre = {&TYPECONTRAT-bail} 
                          then truncate(piNumeroContratMaitre / 100000, 0)
                          else piNumeroContratMaitre
        vcNumeroCompte  = if pcTypeIdent = {&TYPEROLE-locataire}
                          then string(piNumeroIdent modulo 100000, "99999")
                          else string(piNumeroIdent, "99999")
    .
...
procedure bquRlctt:
    ...
    Notes  : service externe (mutbai01.p, contractantContrat.p, indivisaireContrat.p)
    ------------------------------------------------------------------------------*/
    ...
    for each ttRlctt where ttRlctt.CRUD = "":
        ...
        if available rlctt then do:
            ...
            if rlctt.noct2 <> ttRlctt.noct2 then do:
        ...
        and can-find(first intnt no-lock
                     where intnt.tpidt = vcTpidtColocOuMandat
                       ...
                       and intnt.nocon = ttRlctt.noct1) then do:
            create vbttRlctt.
            assign
            ...
            if available rlctt then do:
                //lien banque existe mais banque differente. Dans ce cas on supprime le lien existant car le numero est dans la cle de l'enregistrement
                if rlctt.noct2 <> vbttRlctt.noct2 then do: 
                    // passage du crud a C pour creation lien avec nouvelle banque

// todo  n'est-ce pas plutôt ttRlctt.CRUD = "C". ???
                    vbttRlctt.CRUD = "C".
