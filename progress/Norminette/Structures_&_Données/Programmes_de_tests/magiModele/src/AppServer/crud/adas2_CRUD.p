...
derniere revue: 2018/06/05 - phm: 
gga todo voir si apres ajout index il faut regenerer les moulinettes ?    
------------------------------------------------------------------------*/

{oerealm/include/instanciateTokenOnModel.i}      // Doit être positionnée juste après using
define variable ghttadas2 as handle no-undo.     // le handle de la temp table à mettre à jour

function getIndexField returns logical private(phBuffer as handle, output phNomdt as handle, output phCdexe as handle, output phNofou as handle):
    /*------------------------------------------------------------------------------
...
procedure getAdas2:
    ...
    then for each adas2 no-lock
        where adas2.cdexe = piCdexe:
        vhttBuffer:buffer-create().
        outils:copyValidField(buffer adas2:handle, vhttBuffer).  // copy table physique vers temp-table
    end.
    else for each adas2 no-lock
        where adas2.cdexe = piCdexe
          and adas2.nomdt = piNomdt:
        vhttBuffer:buffer-create().
        outils:copyValidField(buffer adas2:handle, vhttBuffer).  // copy table physique vers temp-table
    end.
    delete object phttAdas2 no-error.
...
procedure updateAdas2 private:
    ...
    getIndexField(vhttBuffer, output vhNomdt, output vhCdexe, output vhNofou).
blocTrans:
    do transaction:
        repeat:
            vhttquery:get-next().
            ...
            if outils:isUpdated(buffer adas2:handle, 'nomdt/cdexe/nofou: ', substitute('&1/&2/&3', vhNomdt:buffer-value(), vhCdexe:buffer-value(), vhNofou:buffer-value()), vhttBuffer::dtTimestamp)
