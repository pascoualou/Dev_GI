...
derniere revue: 2018/08/08 - phm: KO
        todo  pourquoi trtSupPeriodeCharge est appelé sur deletePeriodeCharge et pas sur deletePerio
        todo  il faut lancer les xx_CRUD.p pour les suppressions
        todo  vérifier les modifications faites/proposées
------------------------------------------------------------------------*/
...
blocTrans:
    do transaction:
        repeat:
            ...
            // todo  run trtSupPeriodeCharge(buffer perio). 
...
procedure deleteExerciceCharge:
    /*------------------------------------------------------------------------------
    Purpose: lecture des periodes de charge pour appel suppression
             reprise adb/lib/supchloc.p
    Notes  : service externe 
...
message "trtSupPeriodeCharge " perio.tpctt "// " perio.nomdt "// " perio.noper "// " perio.noexo.
    ...
    for each lprtb exclusive-lock
        ...
        case lprtb.tpcpt:
            when {&TYPETACHE-ImputParticuliereGerance} then do:
    ...
    for each eprov exclusive-lock
        where eprov.tpctt = perio.tpctt
          and eprov.nomdt = perio.nomdt
          and eprov.noexo = perio.noper:
    ...
    viNoPseudoCtratPrestation = perio.nomdt * 100 + perio.noper.     //integer( string(piNumeroMandat,"99999") + STRING(piNumeroPeriode,"99") )
    ...
    for each trfpm exclusive-lock
        where trfpm.tptrf = "PS"
          and trfpm.nomdt = perio.nomdt
          and trfpm.noexe = perio.noper:
    ...
    for each trfev exclusive-lock
        where trfev.tptrf = "PS"
          and trfev.nomdt = perio.nomdt
          and trfev.noexe = perio.noper:
    ...
    for each regul exclusive-lock
        where regul.nomdt = perio.nomdt
          and regul.noper = perio.noper:
    ...
    for each chglo exclusive-lock
        where chglo.tpmdt = perio.tpctt
          and chglo.nomdt = perio.nomdt
          and chglo.noexo = perio.noper:
    ...
    for each solps exclusive-lock
        where solps.tpctt = perio.tpctt
          and solps.nomdt = perio.nomdt
          and solps.noexo = perio.noper:
    ...
    for each vbperio exclusive-lock
        where vbperio.TpCtt = perio.tpctt
          and vbperio.Nomdt = perio.nomdt
          and vbperio.Noexo = perio.noper
          and vbperio.Noper > 0:
