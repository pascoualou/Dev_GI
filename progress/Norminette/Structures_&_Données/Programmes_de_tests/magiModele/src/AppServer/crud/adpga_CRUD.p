...
Notes       : Ancienne table historique Dépôt de garantie, cette table n'est plus utilisée depuis 1997
              un seul index : ix_adpga01  (PUA) = nomdt(A) + noloc(A) + nolig(A)
...
procedure deleteAdpgaSurMandat:
    /*------------------------------------------------------------------------------
...
blocTrans:
    do transaction:
        for each adpga exclusive-lock
            where adpga.nomdt = piNumeroMandat:
            delete adpga no-error.
            if error-status:error then do:
                mError:createError({&error}, error-status:get-message(1)).
                undo blocTrans, leave blocTrans.
