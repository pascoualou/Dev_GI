        ...
        for each iprd exclusive-lock
           where iprd.soc-cd  = piSociete
            ...
            if can-find(first ccptmvt no-lock
                ...
                mError:createError({&error}, substitute("La période du &1 au &2 ne peut pas être modifiée car elle est mouvementée", iprd.dadebprd, iprd.dafinprd)).
                undo blocTrans, leave blocTrans.
            end.
...
procedure creationNouvellesPeriodes:
    /*------------------------------------------------------------------------------
    Purpose: Création de nouvelles périodes dans un exercice comptable
    Notes  : service externe
    ------------------------------------------------------------------------------*/
    define input parameter piSociete      as integer no-undo.
    define input parameter piCodeEtabl    as integer no-undo.
    ...
    do transaction:
        for each isoc no-lock
            ...
            if isoc.specif-cle = 1000 then do:
                //Si pas de numéro d'exercice en entrée, on prend le derier + 1
                ...
                if piNumeroExercice = ? then piNumeroExercice = 5. //On commence à 5 pour garder de la marge afin de pouvoir créer ultérieurement des exercices antérieurs
...
procedure creationPeriode private:
    /*------------------------------------------------------------------------------
    Purpose: Création iprd et ijouprd 
    Notes  : 
    ------------------------------------------------------------------------------*/
