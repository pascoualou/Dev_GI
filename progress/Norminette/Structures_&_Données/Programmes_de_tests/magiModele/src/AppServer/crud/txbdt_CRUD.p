...
procedure deleteTxbdtSurMandantMandat:
    /*------------------------------------------------------------------------------
    Purpose: suppression de tous les enregistrements correspondant aux parametres de selection 
    Notes  : service externe
    ------------------------------------------------------------------------------*/
    define input parameter piNumeroMandant as integer no-undo.
...
blocTrans:
    do transaction:
        for each txbdt exclusive-lock  
            where txbdt.noman = piNumeroMandant
              and txbdt.nomdt = piNumeroMandat:
            delete txbdt no-error.
            if error-status:error then do:
                mError:createError({&error}, error-status:get-message(1)).
                undo blocTrans, leave blocTrans.
        ...
        for each txbet exclusive-lock
            where txbet.noman = piNumeroMandant:
...
procedure deleteTxbdtSurMandat:
    /*------------------------------------------------------------------------------
    Purpose: suppression de tous les enregistrements correspondant aux parametres de selection 
    Notes  : service externe
    ------------------------------------------------------------------------------*/
...
blocTrans:
    do transaction:
        ...
        for each txbdt exclusive-lock  
            where txbdt.nomdt = piNumeroMandat:
            delete txbdt no-error.
            if error-status:error then do:
                mError:createError({&error}, error-status:get-message(1)).
                undo blocTrans, leave blocTrans.
        ...
        if not can-find(first txbdt no-lock
                        where txbdt.noman = piNumeroMandant)
        then for each txbet exclusive-lock
                where txbet.noman = piNumeroMandant:
            delete txbet no-error.
            if error-status:error then do:
                mError:createError({&error}, error-status:get-message(1)).
