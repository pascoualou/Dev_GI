...
derniere revue: 2018/09/03 - phm: OK
------------------------------------------------------------------------*/
...
procedure supLog-iAction-ident:
    /*------------------------------------------------------------------------------
    Purpose: Suppression de toute trace d'action pour un identifiant (ex : "EVENT" + no evenement)
    Notes  : service externe, utilisé dans la suppression des évèvements (GesEveOb.p) 
             a partir de comm/prclogia.i  
    ------------------------------------------------------------------------------*/    
    ...
    if pcCodeAction-IN > ""
    then for each iaction exclusive-lock        /* Suppression des traces d'action d'un code action donné pour cet identifiant */
        where iaction.nomprg = pcCodeAction-IN
          and iaction.tpidt  = pcTypeIdent-IN
          and iaction.noidt  = piNumeroIdent-IN:
        delete iaction.
    end.
    else for each iaction exclusive-lock        /* Suppression de toute trace d'action pour cet identifiant */
