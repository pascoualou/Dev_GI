...
function getNextNoTxt returns integer private (pcTypeRole as character, piNumeroRole as int64):
    /*------------------------------------------------------------------------------
    Purpose:
    Notes  :
    ------------------------------------------------------------------------------*/
    ...
    for last txrole no-lock
       where txrol.tprol = pcTypeRole
         and txrol.norol = piNumeroRole:
        return (txrol.notxt + 1).
    end.
    return 1.
...
procedure createTxrole private:
    ...
    define variable vhNorol    as handle  no-undo.    
    define variable vhNotxt    as handle  no-undo.
    define variable viNotxt    as integer no-undo.
    define buffer txrole for txrole.
...
blocTrans:
    do transaction:
        repeat:
            ...
            viNotxt = vhNotxt:buffer-value().
            if viNotxt = 0
            then vhNotxt:buffer-value() = getNextNoTxt(vhTprol:buffer-value(), vhNorol:buffer-value()).
