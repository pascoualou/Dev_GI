...
procedure setNextChronoFact:
    /*------------------------------------------------------------------------------
    Purpose: Assign et renvoie le prochain chrono interne de facture  
    Notes  : service externe.
    ------------------------------------------------------------------------------*/
    define output parameter piNextChronoFact as integer no-undo.
    define buffer aparm for aparm.
    
    empty temp-table ttAparm.
    find aparm exclusive-lock 
        where aparm.tppar   = "TFACT" 
          and aparm.soc-cd  = 0       
          and aparm.etab-cd = 0       
          and aparm.cdpar   = "PROV"
          no-error.
    create ttAparm.                  
    if not available aparm 
    then assign ttAparm.tppar   = "TFACT"
                ttAparm.soc     = 0
                ttAparm.etab-cd = 0
                ttAparm.cdpar   = "PROV"
                ttAparm.zone1   = 1
         ...
         .
            ...
            ttAparm.zone1       = aparm.zone1 + 1
            ttAparm.CRUD        = "U"
            ttAparm.rRowid      = rowid(aparm)
            ttAparm.dtTimestamp = datetime(aparm.damod, aparm.ihmod)
         .
    piNextChronoFact = ttAparm.zone1.
    run setAparm(table ttAparm by-reference).
end.             
