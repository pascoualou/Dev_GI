...
{immeubleEtLot/include/local.i}
{immeubleEtLot/include/lotSimplifie.i}
...
boucle1:
    repeat:
        ...
        viNumeroBien = vhBuffer::iNumeroImmeuble.
...
boucle1:
    repeat:
        ...
        assign
            viNumeroBien  = vhBuffer::iNumeroBatiment
            viNumeroLigne = vhBuffer::iNumeroLigne
        .
        vhQuerySurface:query-prepare(substitute('FOR EACH &1 where &1.iNumeroBien=&2 and &1.iNumeroLigne=&3', vhttSurface:name, viNumeroBien, viNumeroLigne)).
        vhQuerySurface:query-open().
boucle2:
        repeat:
            vhQuerySurface:get-next().
...
procedure regroupeSurface private:
    ...
    assign
        ...
        pdSurface01   = 0 when pdSurface01 = ?
        pdSurface02   = 0 when pdSurface02 = ?
        pdSurface03   = 0 when pdSurface03 = ?
...
procedure controleSurface:
    ...
    Notes   : d'après PROCEDURE ControlSurf (surface.i)
    todo    : pas utilisé pour le moment. devrait être utilisé pour les surfaces immeuble et lot
    -------------------------------------------------------------------------*/
    define input  parameter pdSurface01    as decimal   no-undo.
    define input  parameter pdSurface02    as decimal   no-undo.
    define input  parameter pdSurface03    as decimal   no-undo.
    ...
    case pcUniteSurface:
        when {&UNITESURFACE-m2} then if (pdSurface01 > 0 or pdSurface02 > 0) and pdSurface03 >= {&Million} then assign
            piErreur = 3
            pcTranche = outilTraduction:getLibelle(1000884) // "doit être < 1 000 000"
            pcUnite = "M2"
        .
        when {&UNITESURFACE-cm2} then if (pdSurface01 > 0 or pdSurface02 > 0) and pdSurface03 >= {&DixMille} then assign
            piErreur = 3
            pcTranche = outilTraduction:getLibelle(1000885) // "doit être < 10 000"
            pcUnite = "CM2"
        .
        when {&UNITESURFACE-are} then if pdSurface02 > 0 and pdSurface03 >= {&Cent} then assign
            piErreur = 3
            pcTranche = outilTraduction:getLibelle(1000886)  //  "doit être < 100"
            pcUnite = "A"
        .
