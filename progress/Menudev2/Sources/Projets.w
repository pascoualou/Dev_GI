...
DO:
    ...
    gSauvePreference("PROJETS_DERNIER_DOCUMENT",ttDocuments.crepertoire + "|" + ttDocuments.cnom).
END.
...
DO:
    ...
    gcAllerRetour = STRING(giPosXMessage)
        ...
        + "|" + gDonnePreference("PROJETS_REPERTOIRE_PERE").
    RUN VALUE(gcRepertoireExecution + "saisie2.w") (INPUT-OUTPUT gcAllerRetour).
    IF gcAllerRetour = "" THEN RETURN.
    gSauvePreference("PROJETS_REPERTOIRE_PERE",ENTRY(4,gcAllerRetour,"|")).
    RUN Initialisation.
END.
...
DO:
    ...
    /* sauvegarde du dernier utilisé */
    gSauvePreference("PROJETS_DERNIER_FICHIER",ttfichiers.crepertoire + "|" + ttfichiers.cnom).
END.
...
DO:
    ...
    /* sauvegarde du dernier utilisé */
    gSauvePreference("PROJETS_DERNIER_REPERTOIRE",ttrepertoires.cRepertoire).
END.
...
DO:
  gSauvePreference("PROJETS_DOCUMENTS_REPERTOIRE",STRING(tglDocuments:CHECKED)).
...
DO:
    ...
    gSauvePreference("PROJETS_PROJETS_REPERTOIRE",STRING(tglProjets:CHECKED)).
...
------------------------------------------------------------------------------*/
    ...
    IF gDonnePreference("PROJETS_REPERTOIRE_PERE") <> "" THEN DO:
        OS-COMMAND SILENT VALUE(gcRepertoireRessourcesPrivees + "\scripts\general\ListeR.bat ~"" + gDonnePreference("PROJETS_REPERTOIRE_PERE") + "~" ~"" + gcRepertoireTempo + "~"").
        ...
        REPEAT:
            ...
            ttRepertoires.cNom = replace(cLigne,gDonnePreference("PROJETS_REPERTOIRE_PERE") + "\","").
        END.
        INPUT CLOSE.
    END.
    ...
    IF gDonnePreference("PROJETS_REPERTOIRE_PERE") <> "" THEN DO:
        OS-COMMAND SILENT VALUE(gcRepertoireRessourcesPrivees + "\scripts\general\ListeF.bat ~"" + gDonnePreference("PROJETS_REPERTOIRE_PERE") + "~" ~"" + gcRepertoireTempo + "~" " + "*.prj").
    ...
    IF gDonnePreference("PROJETS_REPERTOIRE_PERE") <> "" THEN DO:
        cListeFiltres = gDonnePreference("PREF-FILTRESPROJETS").
        OS-COMMAND SILENT VALUE(gcRepertoireRessourcesPrivees + "\scripts\general\ListeD.bat ~"" + gDonnePreference("PROJETS_REPERTOIRE_PERE") + "~" ~"" + gcRepertoireTempo + "~" " + "*.*").
    ...
    DO WITH FRAME frmFonction:
        tglDocuments:CHECKED = (gDonnePreference("PROJETS_DOCUMENTS_REPERTOIRE") = "YES").
        tglProjets:CHECKED = (gDonnePreference("PROJETS_PROJETS_REPERTOIRE") = "YES").
    END.
    ...
    cTempo = gDonnePreference("PROJETS_DERNIER_REPERTOIRE").
    IF  cTempo <> "" THEN DO:
    ...
    END.
    cTempo = gDonnePreference("PROJETS_DERNIER_FICHIER").
    IF  cTempo <> "" THEN DO:
    ...
    END.
    cTempo = gDonnePreference("PROJETS_DERNIER_DOCUMENT").
    IF  cTempo <> "" THEN DO:
