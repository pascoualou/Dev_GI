...
DEFINE VARIABLE filRepertoireCommentaires AS CHARACTER FORMAT "X(256)":U 
     LABEL "Répertoire de stockage des commentaires" 
     VIEW-AS FILL-IN NATIVE 
     SIZE 55 BY .95 NO-UNDO.

DEFINE VARIABLE tglActionsClassees AS LOGICAL INITIAL no 
     LABEL "Une action 'A Faire' vient en tête de liste, une 'Faite' ou 'Abandonnée' passe en fin de liste" 
     VIEW-AS TOGGLE-BOX
     SIZE 90 BY .95 NO-UNDO.

DEFINE VARIABLE tglNomAction AS LOGICAL INITIAL no 
     LABEL "Utiliser le nom de l'action plutôt qu'un numéro pour le fichier commentaire d'une action" 
     VIEW-AS TOGGLE-BOX
     SIZE 87 BY .95 NO-UNDO.

DEFINE VARIABLE tglRetourDebut AS LOGICAL INITIAL no 
     LABEL "Retour au début de la liste après action 'Faite'" 
     VIEW-AS TOGGLE-BOX
...
ON LEAVE OF filRepertoireCommentaires IN FRAME frmModule /* Répertoire de stockage des commentaires */
DO:
      gSauvePreference("PREF-REPERTOIRE-COMMENTAIRES",SELF:SCREEN-VALUE).
END.
...
ON VALUE-CHANGED OF tglActionsClassees IN FRAME frmModule /* Une action 'A Faire' vient en tête de liste, une 'Faite' ou 'Abandonnée' passe en fin de liste */
DO:
    gSauvePreference("PREF-ACTION-DEBUT-FIN",(IF SELF:CHECKED THEN "OUI" ELSE "NON")).
END.
...
ON VALUE-CHANGED OF tglNomAction IN FRAME frmModule /* Utiliser le nom de l'action plutôt qu'un numéro pour le fichier commentaire d'une action */
DO:
    gSauvePreference("PREF-ACTION-COMMENTAIRE-NOM",(IF SELF:CHECKED THEN "OUI" ELSE "NON")).
END.
...
ON VALUE-CHANGED OF tglRetourDebut IN FRAME frmModule /* Retour au début de la liste après action 'Faite' */
DO:
    gSauvePreference("PREF-ACTION-RETOUR-DEBUT",(IF SELF:CHECKED THEN "OUI" ELSE "NON")).
END.
...
------------------------------------------------------------------------------*/
    ...
    DO WITH FRAME frmModule:
        tglActionsClassees:CHECKED = (IF gDonnePreference("PREF-ACTION-DEBUT-FIN") = "OUI" THEN TRUE ELSE FALSE).
        tglNomAction:CHECKED = (IF gDonnePreference("PREF-ACTION-COMMENTAIRE-NOM") = "OUI" THEN TRUE ELSE FALSE).
        tglRetourDebut:CHECKED = (IF gDonnePreference("PREF-ACTION-RETOUR-DEBUT") = "OUI" THEN TRUE ELSE FALSE).
        
        IF gDonnePreference("PREF-REPERTOIRE-COMMENTAIRES") = "" THEN gSauvePreference("PREF-REPERTOIRE-COMMENTAIRES",loc_log).        
        filRepertoireCommentaires:SCREEN-VALUE = gDonnePreference("PREF-REPERTOIRE-COMMENTAIRES").
