...
DO:
    ...
    IF available(agenda) THEN RUN gDeclencheAgenda(agenda.cIdent,"").
END.
...
DO:
  IF gDonnePreference("PREF-TITREMESSAGE") = "OUI" AND edttexte:SCREEN-VALUE = "" THEN
    ...
    DO iBoucle = 1 TO NUM-ENTRIES(cOrdre-in):
        ...
        CASE cAction:
            WHEN "AFFICHE" THEN DO:
                ...
                IF gGetEtSupParam("AGENDA-RECHARGER") = "OUI" THEN DO:
                    RUN Recharger.
                END.
                RUN Affichage.
            END.
...
DEFINE INPUT PARAMETER cEtat-in AS CHARACTER.
    ...
    IF cEtatEncours = "CRE" THEN DO WITH FRAME frmModule :
        IF gDonnePreference("PREF-ALERTEJOUR") = "OUI" THEN 
    ...
    FOR EACH Alarmes NO-LOCK
        ...
        IF Alarmes.ddate = TODAY THEN DO:
            IF Alarmes.iheure = giHeure THEN DO:
                RUN gDeclencheAgenda(Alarmes.cIdent,Alarmes.cIdentAlarme).
            END.
        END.
    END.
