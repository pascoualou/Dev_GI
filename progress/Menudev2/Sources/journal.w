...
DO:
  ...
  DO WITH FRAME Frame-A:
  DO WITH FRAME Frame-B:
      lSauvegarde = (gDonnePreference("PREF-RECHERCHE-JOURNAL-SAUVEGARDE") = "OUI").
...
DO:
    ...
    lAvertissement = (gDonnePreference("PREF-HISTORIQUE-PREVENIR") = "OUI").
...
------------------------------------------------------------------------------*/
    ...
    IF gDonnePreference("PREF-LIMITE-JOURNAL") = "NON" 
...
DEFINE VARIABLE cFichierEdition AS CHARACTER NO-UNDO INIT "".
    ...
    DO WITH FRAME Frame-B:
        ...
        RUN gImpressionFichier(cFichierEdition,"Journal").
    END.
    END.
...
------------------------------------------------------------------------------*/
    ...
    DO WITH FRAME frame-B:
        ...
        IF gDonnePreference("PREF-LIMITE-JOURNAL") = "NON"  THEN DO:
...
------------------------------------------------------------------------------*/
    ...
    DO dDateEnCours = dDateDepart TO TODAY:
        ...
        IF gJourFerie(dDateEnCours) THEN NEXT.
        FIND FIRST  journal NO-LOCK
            WHERE   journal.cutilisateur = gcUtilisateur
            AND     journal.date-svg = ?
