...
------------------------------------------------------------------------------*/
    ...
    IF agenda.laction THEN DO:
        ...
        cActionModifiee = gRemplaceVariables(agenda.cAction,"*",?,0).
    ...
    IF agenda.cSon <> "" AND agenda.cSon <> "-" THEN DO:
        IF not(gDonnePreference("PREF-MODESILENCIEUX")) = "OUI" THEN RUN JoueSon(gcRepertoireRessources + agenda.cSon + ".wav").
    END.
