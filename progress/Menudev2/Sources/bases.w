...
DEFINE TEMP-TABLE ttBases
    ...
    FIELD cCheminBasesLibelle AS CHARACTER
...
DEFINE VARIABLE lMenu                       AS LOGICAL      EXTENT 6.
...
DEFINE VARIABLE lActionManuelle             AS LOGICAL      INIT TRUE.
...
&Scoped-define FIELDS-IN-QUERY-brwBases ttBases.cBase ttBases.cServeur ttBases.lAuto ttBases.ldemarree ttBases.lno-Integrity ttBases.lbase ttBases.lsauvegarde ttBases.cVersion ttBases.cProgress ttBases.cRepertoire ttBases.dDateSauvegarde ttBases.cCommentaire ttbases.cCheminBasesLibelle   
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brwBases C-Win _FREEFORM
  QUERY brwBases DISPLAY
      ttBases.cBase FORMAT "x(15)" WIDTH-PIXELS 50 LABEL "Nom"
          ...
          ttbases.cCheminBasesLibelle FORMAT "x(40)" LABEL "Chemin"
...
DO:
  ...
  IF ttbases.lbase AND ((ttbases.cProgress <> "" AND gcVersionProgress = ttbases.cProgress) OR ttbases.cProgress = "") THEN DO:
      IF ttBases.lDemarree THEN DO:
          RUN gAfficheMessageTemporaire("Confirmation...","Confirmez-vous l'arrêt des serveurs sur la base en cours ?",TRUE,5,"NON","",FALSE,OUTPUT cRetour).
...
DO:
  ...
  ttbases.cCheminBasesLibelle:BGCOLOR IN BROWSE brwbases = iCouleurLigne.
  ...
  IF iCouleurLigne = 12 THEN DO:
      ...
      ttbases.cCheminBasesLibelle:FGCOLOR IN BROWSE brwbases = 15.
  END.
...
DO:
    ...
    IF AVAILABLE(ttbases) THEN DO:
        ...
        gSauvePreference("DERNIERE-BASE",ttbases.cserveur + "," + ttbases.cBase).
...
DO:
    ...
    cCommandeShell = "explorer.exe " + gDonnePreferenceGenerale("PREFS-GENE-ANCIENNES-SAUVEGARDES").
...
DO:
    ...
    RUN gAfficheMessageTemporaire("Gestion des bases",cLibelle,TRUE,10,"NON","",FALSE,OUTPUT cRetour).
    IF cRetour = "OUI" THEN DO:
        /* Mise à non de tous les filtres */
        gSauvePreference("FILTRE-BASEPRESENTE","NON").
        gSauvePreference("FILTRE-BASEABSENTE","NON").
        gSauvePreference("FILTRE-BASEOUVERTE","NON").
        gSauvePreference("FILTRE-BASEFERMEE","NON").
        gSauvePreference("FILTRE-SAUVEGARDEPRESENTE","NON").
        gSauvePreference("FILTRE-SAUVEGARDEPRESENTE","NON").
        gSauvePreference("FILTRE-UTILISATEUR","-").
...
DO:  
    gAddParam("FICHIERS-INFOSFICHIER","bases" + ",VISU").
...
DO:  
    gSauvePreference("FILTRE-UTILISATEUR",SELF:SCREEN-VALUE).
...
DO:
    ...
    IF ttbases.cServeur = "Local" THEN DO:
        IF ttbases.lLib THEN DO:
            cTempo = ttbases.cCheminBasesLibelle.
    ...
    END.
    RUN gAfficheMessageTemporaire("commande en cours","Un avertissement sera envoyé dans la zone information de menudev2 quand la commande sera terminée.",FALSE,5,"OK","MESSAGE-ARRETER",FALSE,OUTPUT cRetour).
...
DO:
    gaddParam("AVEC_VERSION_PROGRESS","OUI").
...
DO:
    ...
    END.
    RUN gAfficheMessageTemporaire("Commande en cours","Un avertissement sera envoyé dans la zone information de menudev2 quand la commande sera terminée.",FALSE,5,"OK","MESSAGE-DEMARRER",FALSE,OUTPUT cRetour).
    ...
    IF not(ttbases.lLib) AND gDonnePreference("PREF-CLIENTINI") = "OUI" THEN DO:
...
DO:
    IF NOT(AVAILABLE(ttBases)) THEN RETURN NO-APPLY.
    IF ttbases.llib THEN DO:
        OS-COMMAND SILENT value("explorer.exe " + ttBases.cCheminBasesLibelle).
    END.
    ELSE DO:
...
DO:
    ...
    END.
    RUN gAfficheMessageTemporaire("commande en cours","Un avertissement sera envoyé dans la zone information de menudev2 quand la commande sera terminée.",FALSE,5,"OK","MESSAGE-FORCER-ARRETER",FALSE,OUTPUT cRetour).
...
DO:
    ...
    IF ttbases.cServeur = "Local" THEN DO:
        ...
        cParam = (IF ttbases.cRepertoire = "Dev" THEN "" ELSE ttbases.cRepertoire).
        RUN ExecuteCommandeDos(gcRepertoireRessourcesPrivees + "\scripts\serveurs\_gencnx.bat " + cRepertoireBases + " "  +  ttbases.cbase + " " + cParam).
    ...
    ELSE DO:
        ...
        REPEAT:
            ...
            IF (gDonnePreference("PREF-CNXPASLOCAL") = "OUI") THEN DO:
        ...
        IF NOT(gDonnePreference("PREF-CNXPASLOCAL") = "OUI") THEN DO:
            cParam = (IF ttbases.cRepertoire = "Dev" THEN "" ELSE ttbases.cRepertoire).
            IF cparam = "" THEN cparam = "CLI".
            RUN ExecuteCommandeDos(gcRepertoireRessourcesPrivees + "\scripts\serveurs\_gencnxLocal.bat " + cRepertoireBases + " "  +  ttbases.cbase + " " + cParam).
        END.

    END.
    
    /* Copie du fichier adb au bon endroit */
    IF gDonnePreference("REPERTOIRES-APPLI") <> "" THEN DO:
        DO iboucle = 1 TO NUM-ENTRIES(gDonnePreference("REPERTOIRES-APPLI")):
            RUN ExecuteCommandeDos("copy " + loc_tmp + "\" + cFichierAdb + " "  + ENTRY(iboucle,gDonnePreference("REPERTOIRES-APPLI"))).
        END.
    END.
    ELSE DO:
        /* Suppression par défaut dans tous les répertoires */
        OS-DELETE VALUE(disque + "gidev" + "\" + cFichierAdb).
        OS-DELETE VALUE(disque + "gi_PREC\gi" + "\" + cFichierAdb).
        OS-DELETE VALUE(disque + "gi" + "\" + cFichierAdb).
        OS-DELETE VALUE(disque + "gi_SUIV\gi" + "\" + cFichierAdb).
        OS-DELETE VALUE(disque + "gi_SPE\gi" + "\" + cFichierAdb).
        
        /* Création dans le bon répertoire */
        IF ttbases.cRepertoire = "Dev" THEN
            cOuCopier = "gidev".
        ELSE IF ttbases.cRepertoire = "CLI" THEN
            cOuCopier = "gi".
        ELSE IF ttbases.cRepertoire = "SUIV" THEN
            cOuCopier = "gi_SUIV\gi".
        ELSE IF ttbases.cRepertoire = "PREC" THEN
            cOuCopier = "gi_PREC\gi".
        ELSE IF ttbases.cRepertoire = "SPE" THEN
            cOuCopier = "gi_SPE\gi".
        ELSE cOuCopier = "gidev".
    
        cOuCopier = disque + cOuCopier.
    ...
    IF ttbases.cRepertoire <> "Dev" THEN DO:
        ...
        lBasesLibFixes = FALSE
                        OR (ttbases.cRepertoire = "PREC" AND gDonnePreference("PREF-MAGICONNEXPREC") = "oui")
                        OR (ttbases.cRepertoire = "CLI" AND gDonnePreference("PREF-MAGICONNEXCLI") = "oui")
                        OR (ttbases.cRepertoire = "SUIV" AND gDonnePreference("PREF-MAGICONNEXSUIV") = "oui")
                        OR (ttbases.cRepertoire = "SPE" AND gDonnePreference("PREF-MAGICONNEXSPE") = "oui")
                        .
        IF lBasesLibFixes THEN DO:
            ...
            PUT STREAM sSortie UNFORMATTED gDonnePreference("PREF-MAGICONNEXLIB" + ttbases.cRepertoire) SKIP.
...
DO:
    ...
    IF ttbases.cServeur = "Local" THEN DO:
        ...
        END.
    END.
    RUN gAfficheMessageTemporaire("Commande en cours","Un avertissement sera envoyé dans la zone information de menudev2 quand la commande sera terminée.",FALSE,5,"OK","MESSAGE-LK",FALSE,OUTPUT cRetour).
END.
...
DO:
    ...
    gSauvePreference("DERNIERE-BASE","Local" + "," + ENTRY(4,gcAllerRetour,"|")).
    RUN Recharger.
END.
...
DO:
    ...
    END.
    RUN gAfficheMessageTemporaire("Restauration en cours","Un avertissement sera envoyé dans la zone information de menudev2 quand la restauration sera terminée.",FALSE,5,"OK","MESSAGE-RESTAURER",FALSE,OUTPUT cRetour).
END.
...
DO:
    ...
    IF cRetour = "NON" THEN RETURN NO-APPLY.
    IF ttbases.cServeur = "Local" THEN DO:
        RUN ExecuteCommandeDos(gcRepertoireRessourcesPrivees + "\scripts\serveurs\_GI-Sauvegarde.bat " + cRepertoireBases + " "  + ttBases.cbase + " " + (IF toggle-1:CHECKED IN FRAME frmfonction THEN "pause" ELSE "rem") + " -mx" + gDonnePreference("PREF-COMPRESSION")).
        RUN gAfficheMessageTemporaire("Sauvegarde en cours","Un avertissement sera envoyé dans la zone information de menudev2 quand la sauvegarde sera terminée.",FALSE,5,"OK","MESSAGE-SAUVEGARDER",FALSE,OUTPUT cRetour).
    END.
    ELSE DO:
...
DO:
    ...
    IF gDonnePreference("PREFS-SUPPRIME-CNX-ADB") = "OUI" THEN DO:
    ...
    RUN gAfficheMessageTemporaire("Gestion des bases","Suppression effectuée.",FALSE,5,"OK","MESSAGE-SUPPRIMEBASE",FALSE,OUTPUT cRetour).
    RUN Recharger /*TopChronoGeneral*/.
END.
...
DO:
    ...
    RUN gAfficheMessageTemporaire("Gestion des bases","Suppression effectuée.",FALSE,5,"OK","MESSAGE-SUPPRIMESAUVEGARDE",FALSE,OUTPUT cRetour).
    RUN Recharger /*TopChronoGeneral*/.
END.
...
DO:
    ...
    IF gDonnePreference("PREF-FERMERTOUTES=TOUT") = "OUI" THEN DO:
        FOR EACH bttbasesRef
            ...
            IF bttbasesRef.cServeur = "Local" THEN DO:
                IF bttbasesRef.lLib THEN DO:
                    cTempo = bttbasesRef.cCheminBasesLibelle.
        ...
        FOR EACH bttbases
            ...
            IF bttbases.cServeur = "Local" THEN DO:
                IF bttbases.lLib THEN DO:
                    cTempo = bttbases.cCheminBasesLibelle.
...
DO:
    ...
    gSauvePreference("PREF-BASESTYPE",SELF:SCREEN-VALUE).
    RUN GereTglBases.
    RUN recharger.
END.
...
DO:
    gSauvePreference("FILTRE-ET",(IF SELF:SCREEN-VALUE = "YES" THEN "OUI" ELSE "NON")).
    IF lFiltreToutDeSuite THEN RUN recharger.
END.
...
DO:
    ...
    gSauvePreference("PREF-BASESBARBADE",(IF SELF:CHECKED THEN "OUI" ELSE "NON")).
    lActionManuelle = TRUE.
    RUN Recharger /*TopChronoGeneral*/.
...
DO:
    ...
    gSauvePreference("FILTRE-BASEABSENTE",(IF SELF:CHECKED THEN "OUI" ELSE "NON")).
    IF lFiltreToutDeSuite THEN RUN recharger.
END.
...
DO:
    ...
    gSauvePreference("FILTRE-BASEFERMEE",(IF SELF:CHECKED THEN "OUI" ELSE "NON")).
    IF lFiltreToutDeSuite THEN RUN recharger.
END.
...
DO:
    ...
    gSauvePreference("FILTRE-BASEOUVERTE",(IF SELF:CHECKED THEN "OUI" ELSE "NON")).
    IF lFiltreToutDeSuite THEN RUN recharger.
END.
...
DO:
    ...
    gSauvePreference("FILTRE-BASEPRESENTE",(IF SELF:CHECKED THEN "OUI" ELSE "NON")).
    IF lFiltreToutDeSuite THEN RUN recharger.
END.
...
DO:
    ...
    gSauvePreference("FILTRE-SAUVEGARDEABSENTE",(IF SELF:CHECKED THEN "OUI" ELSE "NON")).
    IF lFiltreToutDeSuite THEN RUN recharger.
END.
...
DO:
    ...
    gSauvePreference("FILTRE-SAUVEGARDEPRESENTE",(IF SELF:CHECKED THEN "OUI" ELSE "NON")).
    IF lFiltreToutDeSuite THEN RUN recharger.
END.
...
DO:
    ...
    gSauvePreference("PREF-BASESLOCALES",(IF SELF:CHECKED THEN "OUI" ELSE "NON")).
    IF SELF:CHECKED AND gDonnePreference("REPERTOIRE-BASES") = "" THEN DO:
        RUN gAfficheMessageTemporaire("Contrôles...","Vous n'avez pas précisé de répertoire pour les bases dans les préférences !",FALSE,5,"OK","MESSAGE-BASELOCALE",FALSE,OUTPUT cRetour).
    END.
    lActionManuelle = TRUE.
    RUN Recharger /*TopChronoGeneral*/.
    APPLY "ENTRY" TO brwbases.
...
DO:
    ...
    gSauvePreference("PREF-BASESNEPTUNE2",(IF SELF:CHECKED THEN "OUI" ELSE "NON")).
    lActionManuelle = TRUE.
    RUN Recharger /*TopChronoGeneral*/.
    APPLY "ENTRY" TO brwbases.
END.
...
DO:
    ...
    gSauvePreference("PREF-BASESOUVERTESENTETE",(IF SELF:CHECKED THEN "OUI" ELSE "NON")).
    lActionManuelle = TRUE.
    RUN Recharger /*TopChronoGeneral*/.
    APPLY "ENTRY" TO brwbases.
END.
...
DO:
    gSauvePreference("PREF-REGROUPERBASES",(IF SELF:CHECKED THEN "OUI" ELSE "NON")).
    lActionManuelle = TRUE.
    RUN Recharger /*TopChronoGeneral*/.
    APPLY "ENTRY" TO brwbases.
END.
...
DEFINE INPUT PARAMETER cAction-in AS CHARACTER NO-UNDO.
    ...
    ELSE DO:
        ...
        REPEAT:
            ...
            ttbases.cTri = (IF gDonnePreference("PREF-TRIBASES") = "OUI" THEN SUBSTRING(ttbases.cbase,2) ELSE ttbases.cbase)
...
------------------------------------------------------------------------------*/
    ...
    gMlog("Version Application - Recherche version : " + ttbases.cBase + " -> " + cFichierversion + " => " + ttBases.cVersion).
    ----*/
...
------------------------------------------------------------------------------*/
    ...
    cRepertoireBases = gDonnePreference("REPERTOIRE-BASES").
...
------------------------------------------------------------------------------*/
    ...
    /* Ouverture du fichier */
    RUN gDechargeFichierEnLocal("","tc_droits.lst").
...
------------------------------------------------------------------------------*/
    ...
    DO WITH FRAME FrmFiltres:
        ...
        tglFiltrePresente:CHECKED = (IF gDonnePreference("FILTRE-BASEPRESENTE") = "OUI" THEN TRUE ELSE FALSE).
        tglFiltreAbsente:CHECKED = (IF gDonnePreference("FILTRE-BASEABSENTE") = "OUI" THEN TRUE ELSE FALSE).
        tglFiltreOuverte:CHECKED = (IF gDonnePreference("FILTRE-BASEOUVERTE") = "OUI" THEN TRUE ELSE FALSE).
        tglFiltreFermee:CHECKED = (IF gDonnePreference("FILTRE-BASEFERMEE") = "OUI" THEN TRUE ELSE FALSE).
        tglFiltreSauvegardePresente:CHECKED = (IF gDonnePreference("FILTRE-SAUVEGARDEPRESENTE") = "OUI" THEN TRUE ELSE FALSE).
        tglFiltreSauvegardeAbsente:CHECKED = (IF gDonnePreference("FILTRE-SAUVEGARDEABSENTE") = "OUI" THEN TRUE ELSE FALSE).
        RsFiltreEt:SCREEN-VALUE = STRING(gDonnePreference("FILTRE-ET") = "OUI" ). 
       ...
       IF rsChoixBases:SCREEN-VALUE IN FRAME frmFonction = "3" THEN DO:
            ...
            ELSE DO:
                ...
                END.
            END.
            cmbUtil:SCREEN-VALUE = gDonnePreference("FILTRE-UTILISATEUR").
        END.
        ELSE DO:           
...
------------------------------------------------------------------------------*/
    ...
    /* Demande de confirmation */
    RUN gAfficheMessageTemporaire("Copie de sauvegarde","Confirmez-vous la copie de la sauvegarde : " + cTempo,TRUE,10,"NON","MESSAGE-COPIE-SAUVEGARDE",FALSE,OUTPUT cRetour).
    IF cRetour = "NON" THEN RETURN.

    /* Récupération des infos de connexion à la machine source */
    IF gDonnePreferenceUtilisateur(ttbases.cServeur,"PREF-AUTORISER-UTILISATEURS") <> "OUI" THEN DO:
        RUN gAfficheMessageTemporaire("Copie de sauvegarde","L'utilisateur n'a pas autorisé la copie des sauvegardes se trouvant sur sa machine !"
        ...
        IF cRetour = "OUI" THEN DO:
            ...
            RUN gEnvoiOrdre("INFOS",cTempo,ttbases.cServeur,gcUtilisateur,FALSE,FALSE).
    ...
    cCopie_Utilisateur = gDonnePreferenceUtilisateur(ttbases.cServeur,"PREF-AUTORISER-UTILISATEURS-UTILISATEUR").
    cCopie_MDP = gDonnePreferenceUtilisateur(ttbases.cServeur,"PREF-AUTORISER-UTILISATEURS-MDP").
    IF cCopie_Utilisateur = "" OR cCopie_MDP = "" 
        THEN DO:
        RUN gAfficheMessageTemporaire("Copie de sauvegarde","L'utilisateur n'a pas saisi son code utilisateur ou son mot de passe pour que vous puissiez vous connecter à sa machine !"
                                       + CHR(10) + "Voulez-vous lui envoyer un message pour lui demander de le faire ?"
    ...
    IF SEARCH(cRepertoireBases + "\" + ttbases.cbase + "\svg\" + ttbases.cbase + ".7z") <> ? THEN DO:
        RUN gAfficheMessageTemporaire("Copie de sauvegarde","Une sauvegarde de ce client existe déjà sur votre poste. Elle sera écrasée ! Confirmez-vous la copie ?",TRUE,10,"NON","MESSAGE-EXISTE-DEJA",FALSE,OUTPUT cRetour).
        IF cRetour = "NON" THEN RETURN.
    END.
    ...
    RUN gEnvoiOrdre("INFOS",cTempo,ttbases.cServeur,gcUtilisateur,FALSE,FALSE).
    RUN ExecuteCommandeDos(cCommandeShell).
    RUN gAfficheMessageTemporaire("Commande en cours","Un avertissement sera envoyé dans la zone information de menudev2 quand la commande sera terminée.",FALSE,5,"OK","MESSAGE-COPIESVG",FALSE,OUTPUT cRetour).
END PROCEDURE.
    ...
    DO iBoucle = 1 TO NUM-ENTRIES(cOrdre-in):
        ...
        CASE cAction:
            WHEN "AFFICHE" THEN DO:
                ...
                IF gGetEtSupParam("BASES-RECHARGER") = "OUI" THEN DO:
                    RUN Recharger.
                END.
            END.
            WHEN "CACHE" THEN DO:
...
------------------------------------------------------------------------------*/
    ...
    lLibAuto = (gDonnePreference("PREF-LIBAUTO") = "OUI").
    ...
    IF AVAILABLE(bttbases) THEN DO:
        RUN gAfficheMessageTemporaire("Confirmation...","Voulez-vous fermer les serveurs sur les bases libellé ?",TRUE,5,"OUI","MESSAGE-SERVEURS-LIBELLE",FALSE,OUTPUT cRetour).
        IF cRetour = "NON" THEN RETURN.
    END.
...
------------------------------------------------------------------------------*/
    ...
    FIND FIRST bttbases
        ...
        NO-ERROR.
    IF AVAILABLE(bttbases) THEN DO:
        RUN gAfficheMessageTemporaire("Confirmation...","Des serveurs sur des bases sont encores ouverts.%sConfirmez vous la fermeture de l'application ?",TRUE,5,"OUI","MESSAGE-SERVEURS",FALSE,OUTPUT cRetour).
        IF cRetour = "NON" THEN RETURN.
    END.
...
------------------------------------------------------------------------------*/
    ...
    cRetour = gDonnePreference("RETOUR-MONTAGE").
    IF cRetour = "" THEN RETURN.
    /* Avertissement */
    RUN gAfficheMessageTemporaire("Montage de version en cours","Un avertissement sera envoyé dans la zone information de menudev2 quand le montage de version sera terminée.",FALSE,5,"OK","MESSAGE-MONTAGE",FALSE,OUTPUT cRetourQuestion).
...
------------------------------------------------------------------------------*/
    ...
    IF gDonnePreference("REPERTOIRE-BASES") <> cRepertoireBases THEN RUN initialisation.
...
------------------------------------------------------------------------------*/
    ...
    IF lMajCnx-in  THEN DO: 
        RUN gAfficheMessageTemporaire("Changement de type de version","Le fichier CNX va être re-généré suite au changement de type de version...",FALSE,5,"OK","MESSAGE-REGENERECNX",FALSE,OUTPUT cRetour).
...
------------------------------------------------------------------------------*/
    ...
    DO WITH FRAME frmFonction:
        tglOuvertes:CHECKED = (gDonnePreference("PREF-BASESOUVERTESENTETE") = "OUI").
        tglLocal:CHECKED = (gDonnePreference("PREF-BASESLOCALES") = "OUI").
        tglBarbade:CHECKED = (gDonnePreference("PREF-BASESBARBADE") = "OUI").
        tglNeptune2:CHECKED = (gDonnePreference("PREF-BASESNEPTUNE2") = "OUI").
        rsChoixBases:SCREEN-VALUE = gDonnePreference("PREF-BASESTYPE").
    END.

    lFiltreToutDeSuite = (gDonnePreference("FILTRE-TOUTDESUITE") = "OUI").
    ...
    FOR EACH bttbasesRef
        ...
        IF bttbasesRef.cServeur = "Local" THEN DO:
            IF bttbasesRef.lLib THEN DO:
                ...
                IF not(gDonnePreference("PREF-LIBAUTO") = "OUI") THEN DO:
...
------------------------------------------------------------------------------*/
    ...
    gMlog("RecupereInfosBase - Fichier infos =  " + cFichierInfos-in).
...
DEFINE INPUT PARAMETER cRepertoire-in AS CHARACTER NO-UNDO.
    ...
    END.
    gMlog("Sauve Info : cFichierInfos = " + cFichierInfos).
...
------------------------------------------------------------------------------*/
    ...
    DEFINE VARIABLE cSuffixeRepertoire      AS CHARACTER    NO-UNDO.

    IF not(lActionManuelle) AND gDonnePreference("PREF-RAFRAICHISSEMENTMANUELBASES") = "OUI" THEN RETURN.
    
    IF lActionManuelle THEN AfficheInformations("Veuillez patienter...",0).
    
    lGestionFichiersAdb = (gDonnePreference("PREF-GESTIONFICHIERSADB") = "OUI").
    lOuvertesEnPremier = (gDonnePreference("PREF-BASESOUVERTESENTETE") = "OUI").

    EMPTY TEMP-TABLE ttbases.

    lDoitChargerAuDemarrage = (lPremierPassage AND gDonnePreference("PREFS-VERSION-PROGRESS-DEMARRAGE") = "OUI").
    lAvecVersionProgress = (ggetparam("AVEC_VERSION_PROGRESS") = "OUI").
    gmlog("lDoitChargerAuDemarrage = " + STRING(lDoitChargerAuDemarrage,"OUI/NON") + "%slAvecVersionProgress = " + STRING(lAvecVersionProgress,"OUI/NON")).

    RUN ChargeBasesLib.
    IF (gDonnePreference("PREF-BASESLOCALES") = "OUI" AND  cRepertoireBases <> "") OR rsChoixBases:SCREEN-VALUE IN FRAME frmFonction = "2" THEN DO:
        RUN ChargeBases.
    
        /* Recherche des fichier lk */
        ETIME(TRUE).
        gMlog("Debut Chargement de la liste des bases").
        FOR EACH ttBases:
            lBaseLib = ttBases.lLib.
            IF lBaseLib THEN DO:
                /* Gerer la bascule V10-V11 */
                cSuffixeRepertoire = "".
                IF gcVersionProgress <> "V10" THEN do:
                    cSuffixeRepertoire = suffixe.
                END.
                
                IF ttBases.cbase = "CLI" THEN cTempo = "gi" + cSuffixeRepertoire + "\".
                ELSE IF ttBases.cbase = "DEV" THEN cTempo = "gidev" + cSuffixeRepertoire + "\".
                ELSE cTempo = "gi_" + ttBases.cbase + cSuffixeRepertoire + "\gi\". /* Versions PREC, SUIV, SPE */
                
                cFichierInfosBases = disque + cTempo + "baselib\_infos.mdev2".
                cFichierbase = disque + cTempo + "baselib\ladb.db".
                cFichiersvg = "".
                ttbases.cCheminBasesLibelle = disque + cTempo + "baselib".
            END.
            ELSE DO:
                cFichierInfosBases = cRepertoireBases + "\" + ttBases.cbase + "\_infos.mdev2".
                cFichierbase = cRepertoireBases + "\" + ttBases.cbase + "\inter.db".
                cFichiersvg = cRepertoireBases + "\" + ttBases.cbase + "\svg\" + ttBases.cbase + ".7z".
                ttbases.cCheminBasesLibelle = cRepertoireBases + "\" + ttBases.cbase.
            END.
            ...
            IF gDonnePreference("PREFS-BASES-FICHIERS-NOUVELLE-GESTION-EFFACER-ANCIENS") <> "OUI" THEN DO:
            ...
            IF ttBases.lbase THEN DO:
                IF (lAvecVersionProgress OR lDoitChargerAuDemarrage) THEN DO:
                    IF ttbases.lLib THEN DO:
                        cBaseVersion = ttbases.cCheminBasesLibelle + "\lcompta.db".                
                        cFichierProgress = ttbases.cCheminBasesLibelle + "\" + "_infos.tmpmdev2".
                    END.
                    ELSE DO:
                    ...
                    gMlog(ttBases.cbase + " ---> Version Progress : Calcul = " + ttbases.cProgress).
                END.
                ELSE DO:
                    /* Rechargement depuis le fichier des informations */
                    ttbases.cProgress = cIbVersionProgress.
                    gmlog(ttBases.cbase + " ---> Version Progress : Lecture = " + ttbases.cProgress).
                END.
            END.
            ...
            /* Gestion des adb des bases ouvertes ou fermées */
            IF lGestionFichiersAdb AND NOT(lBaseLib) THEN DO:
                cListeRepertoiresAdb = gDonnePreference("REPERTOIRES-APPLI").
                DO iboucle = 1 TO NUM-ENTRIES(cListeRepertoiresAdb):
                    cFichierAdb = ENTRY(iboucle,cListeRepertoiresAdb) + "\adb" + ttbases.cbase.
                    cFichierAdbCopie = ENTRY(iboucle,cListeRepertoiresAdb) + "\_Fichiers-adb\adb" + ttbases.cbase.
                    IF ttbases.ldemarree THEN 
                        OS-COMMAND SILENT value("move " + cFichierAdbCopie + " " + cFichierAdb).
                    ELSE
                        OS-COMMAND SILENT value("move " + cFichierAdb + " " + cFichierAdbCopie).
                END.
            END.
            ...
            IF ttBases.lbase THEN DO:
                IF ttbases.lLib THEN DO:
                    run SauveInfosBase(ttBases.cBase,ttbases.cCheminBasesLibelle).
                END.
                ELSE DO:
                    run SauveInfosBase(ttBases.cBase,"").
                END.
            END.
        END.
        gMlog("Fin Chargement de la liste des bases : " + string(ETIME)).
        gSauvePreference("PREFS-BASES-FICHIERS-NOUVELLE-GESTION-EFFACER-ANCIENS","OUI").
    
    END.

    /* Chargement des droits de la TC */
    ETIME(TRUE).
    IF gDonnePreference("PREF-BASESBARBADE") = "OUI" 
    OR gDonnePreference("PREF-BASESNEPTUNE2") = "OUI" THEN DO:
        RUN ChargeDroitsTC.
    END.
    gMlog("Chargement des droits de la TC : " + string(ETIME)).

    /* Chargement des bases de Barbade */
    ETIME(TRUE).
    IF gDonnePreference("PREF-BASESBARBADE") = "OUI" THEN DO:
        RUN LectureListeTC("barbade").
    END.
    gMlog("Chargement des bases de Barbade : " + string(ETIME)).
    
    /* Chargement des bases de Neptune2 */
    ETIME(TRUE).
    IF gDonnePreference("PREF-BASESNEPTUNE2") = "OUI" THEN DO:
        RUN LectureListeTC("neptune2").
    END.
    gMlog("Chargement des bases de Neptune2 : " + string(ETIME)).

    /* Chargement des bases des autres utilisateurs */
    ETIME(TRUE).
    RUN BasesUtilisateurs(""). 
    gMlog("Chargement des bases des autres utilisateurs : " + string(ETIME)).
    ...
    release fichiers.
    gMlog("Export des bases : " + string(ETIME)).

    /* Dans tous les cas */
    gsupParam("AVEC_VERSION_PROGRESS").
    ...
    END.
    gMlog("Sauvegarde de la liste dans l'état avant les filtres et l'ouverture de la query : " + string(ETIME)).

    /* Application des filtres */
    ETIME(TRUE).
    IF iNombreFiltres > 0 THEN DO:
        cFiltreUtil = gDonnePreference("FILTRE-UTILISATEUR").
        FOR EACH ttbases:
            iFiltresOk = 0.
        ...
        END.
    END.
    gMlog("Application des filtres : " + string(ETIME)).

    /* ouverture du query */
    ETIME(TRUE).
    RUN OuvreQuery("").
    gMlog("Ouverture de la Query : " + string(ETIME)).
    
    /* repositionnement sur la dernière base utilisée */
    ETIME(TRUE).
    brwbases:SET-REPOSITIONED-ROW(21,"CONDITIONAL") IN FRAME frmFonction.
    cDerniereBase = gDonnePreference("DERNIERE-BASE").
    IF cDerniereBase <> "" AND num-entries(cDerniereBase) = 2 THEN DO:
        ...
        END.
    END.
    gMlog("Repositionnement du browse : " + string(ETIME)).
    ...
    END.
    INPUT STREAM sEntree CLOSE.
    gMlog ("Calcul taille restante sur le disque : " + string(ETIME)).
...
DEFINE VARIABLE cTypeAction AS CHARACTER NO-UNDO.
    ...
    FOR EACH ordres EXCLUSIVE-LOCK
        ...
        IF cTypeAction MATCHES "*INFOS*" THEN DO:
            RUN gEnvoiOrdre("INFOS",ordres.cmessage,ordres.cutilisateur,ordres.filler,false,FALSE).
        END.
...
------------------------------------------------------------------------------*/
    ...
    IF lErreurConnexion OR lErreurAdb THEN DO:
        cErreur = "Attention :%s"
            ...
            .
        RUN gAfficheMessageTemporaire("Problème potentiel...",cErreur,TRUE,10,"NON","",FALSE,OUTPUT cRetour).
        IF cRetour = "OUI" THEN DO:
            APPLY "CHOOSE" TO MENU-ITEM m_Generation IN MENU Menubases.
        END.
    END.
...
------------------------------------------------------------------------------*/
    ...
    IF gDonnePreference("BASES-COULEUR-PROVENANCE") = "OUI" THEN DO:
...
------------------------------------------------------------------------------*/
    ...
    IF gDonnePreference(cPrefixe + "-ACTIF") = "OUI" THEN DO:
        cRetour = gDonnePreference(cPrefixe).  
    END.
    IF cRetour = "" THEN cRetour = gDonnePreference("PREFS-SERVEURS-STANDARD").
