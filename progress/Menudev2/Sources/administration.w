...
DO:
    IF gDonnePreference("TRAVAIL-STATS-CHARGEMENT-EN-COURS") = "" THEN DO:
        RUN gAfficheMessageTemporaire("Administration","Il n'y a aucun blocage du chargement des stats",FALSE,5,"OK","",FALSE,OUTPUT cRetour).
        RETURN NO-APPLY.
    END.

    RUN gAfficheMessageTemporaire("Administration","Confirmez-vous le déblocage du chargement des stats ?",TRUE,10,"NON","",FALSE,OUTPUT cRetour).
...
DO:
    ...
    gAddParam("FICHIERS-INFOSFICHIER",lstFichiers:SCREEN-VALUE + ",PARAM").
...
DO:
    ...
    gMlog("Changement d'utilisateur : "
...
------------------------------------------------------------------------------*/
    ...
    RUN gDechargeFichierEnLocal("","saints.txt").
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE DechargeVariables C-Win 
...
------------------------------------------------------------------------------*/
    ...
    PUT STREAM gstrSortie UNFORMATTED "winGeneral = " + gFormatteValeur(STRING(winGeneral)) SKIP.
    PUT STREAM gstrSortie UNFORMATTED "cParametres = " + gFormatteValeur(STRING(cParametres)) SKIP.
    PUT STREAM gstrSortie UNFORMATTED "cUtilisateur_svg = " + gFormatteValeur(STRING(cUtilisateur_svg)) SKIP.
    PUT STREAM gstrSortie UNFORMATTED "iX = " + gFormatteValeur(STRING(iX)) SKIP.
    PUT STREAM gstrSortie UNFORMATTED "iY = " + gFormatteValeur(STRING(iY)) SKIP.

     PUT STREAM gstrSortie UNFORMATTED FILL("-",80) SKIP.
     PUT STREAM gstrSortie UNFORMATTED SKIP "ttUtil = " SKIP.
     FOR EACH ttUtil:
         PUT STREAM gstrSortie UNFORMATTED "    cUtilisateur = " + gFormatteValeur(string(ttUtil.cUtilisateur)) SKIP.
         PUT STREAM gstrSortie UNFORMATTED "    lAdmin = " + gFormatteValeur(string(ttUtil.lAdmin)) SKIP.
         PUT STREAM gstrSortie UNFORMATTED "    lconnecte = " + gFormatteValeur(string(ttUtil.lconnecte)) SKIP.
         PUT STREAM gstrSortie UNFORMATTED "    lOrdres = " + gFormatteValeur(string(ttUtil.lOrdres)) SKIP.
         PUT STREAM gstrSortie UNFORMATTED "    iNiveau = " + gFormatteValeur(string(ttUtil.iniveau)) SKIP.
         PUT STREAM gstrSortie UNFORMATTED "    iVersion = " + gFormatteValeur(string(ttUtil.iVersion)) SKIP.
         PUT STREAM gstrSortie UNFORMATTED "    cGroupe = " + gFormatteValeur(string(ttUtil.cGroupe)) SKIP.
