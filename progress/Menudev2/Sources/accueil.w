...
DO:
  ...
  /*  */
    ...
    RUN gDonneAbsences(TRUE,OUTPUT cAbsJour, OUTPUT cAbsFutures).
    ...
    DO iBoucle = 1 TO NUM-ENTRIES(cOrdre-in):
        ...
        CASE cAction:
            WHEN "AFFICHE" THEN DO:
                ...
                IF gGetEtSupParam("ACCUEIL-RECHARGER") = "OUI" THEN DO:
                    RUN Recharger.
                END.
...
------------------------------------------------------------------------------*/
    ...
    IF not(gDonnePreference("PREF-MEMOGENE") = "OUI") THEN gcTypeMemo = STRING(DonneNumeroJour(TODAY)).
    ...
    DO WITH FRAME frmagenda:
        filLibFutures:SCREEN-VALUE = "Absence(s) des " + gDonnePreference("PREFS-ABSENCES-JOURS") + " jours à venir".
...
------------------------------------------------------------------------------*/
    ...
    DO WITH FRAME frmAgenda:
        ...
        IF cAbsFutures = "" THEN do:
            cAbsFutures = "Aucune absence signalée pour les " + string(integer(gDonnePreference("PREFS-ABSENCES-JOURS"))) + " prochains jours".
        END.
...
/* Gestion du chrono général */
    ...
    RUN MajListeAlarmes.
     ...
     IF lTopHoraire THEN DO:
         ...
         iDebutRappelHoraire = INTEGER(gDonnePreference("PREF-DEBUTRAPPELHORAIRE")).
         IF iDebutRappelHoraire = 0 THEN iDebutRappelHoraire = 9.
         iFinRappelHoraire = INTEGER(gDonnePreference("PREF-FINRAPPELHORAIRE")).
         ...
         IF ((INTEGER(ENTRY(1,STRING(TIME,"hh:mm:ss"),":")) >= iDebutRappelHoraire AND INTEGER(ENTRY(1,STRING(TIME,"hh:mm:ss"),":")) <= iFinRappelHoraire)) OR glBy-pass THEN DO:
            ...
            FIND FIRST  memo    NO-LOCK
                ...
                NO-ERROR.
            IF AVAILABLE(memo) THEN DO:  
                RUN gDeclencheHoraire(memo.cValeur).
            END.
            /* Memo général */
            FIND FIRST  memo    NO-LOCK
                WHERE   memo.cUtilisateur = gcUtilisateur
            ...
            FOR EACH    AFaire_Action NO-LOCK
               ...
               ,FIRST   AFaire_Liste NO-LOCK
                ...
                :
                gmlog("Action - " + AFaire_Liste.cLibelleListe + "/" + AFaire_Action.cLibelleAction + " : " + AFaire_Liste.cFiller1).
            ...
            IF lAlerteMemo THEN DO:  
                RUN gDeclencheHoraire2(cMessageAFaire).
            END.
         END.
     END.
END PROCEDURE.
