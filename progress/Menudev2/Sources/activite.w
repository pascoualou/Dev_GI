...
DO:
    ...
    IF LOOKUP(SELF:SCREEN-VALUE,cPredefinis) > 0 THEN DO:
        IF not(lPasControle) THEN DO:
            RUN gAfficheMessageTemporaire("Activité","Interdiction de saisir un type d'action prédéfini !%sUtilisez le menu du browse.",FALSE,10,"OK","",FALSE,OUTPUT cRetour).
            RETURN NO-APPLY.
        END.
        lPasControle = FALSE.
    END.
...
DO:
    IF DATE(SELF:SCREEN-VALUE) > TODAY THEN DO:
        RUN gAfficheMessageTemporaire("Activité","La date de début de recherche ne peut pas être dans le futur",FALSE,10,"OK","",FALSE,OUTPUT cRetour).
        SELF:SCREEN-VALUE = STRING(ADD-INTERVAL(TODAY,-2,"months"),"99/99/9999").
    END.
    IF DATE(SELF:SCREEN-VALUE) = ? THEN DO:
        RUN gAfficheMessageTemporaire("Activité","La date de début de recherche est obligatoire",FALSE,10,"OK","",FALSE,OUTPUT cRetour).
        SELF:SCREEN-VALUE = STRING(ADD-INTERVAL(TODAY,-2,"months"),"99/99/9999").
    END.
END.
...
DO:
    ...
    IF lookup(ttActivites.cTypeActivite,"DDJ,FDJ,REPAS") <> 0 THEN DO:
        RUN gAfficheMessageTemporaire("Activité","Vous ne pouvez pas dupliquer ce type d'activité prédéfinie !",FALSE,10,"OK","",FALSE,OUTPUT cRetour).
        RETURN.
    END.
...
DO:
    ...
    IF lookup(ttActivites.cTypeActivite,cPredefinis) <> 0 THEN DO:
        RUN gAfficheMessageTemporaire("Activité","Vous ne pouvez pas reprendre une activité prédéfinie !",FALSE,10,"OK","",FALSE,OUTPUT cRetour).
        RETURN.
    END.
    ...
    DO iBoucle = 1 TO NUM-ENTRIES(cOrdre-in):
        ...
        CASE cAction:
            WHEN "AFFICHE" THEN DO:
                ...
                IF gGetEtSupParam("ACTIVITE-RECHARGER") = "OUI" THEN DO:
...
DEFINE OUTPUT PARAMETER lRetour-ou AS LOGICAL NO-UNDO INIT FALSE.
    ...
    IF cType-in = "DDJ" THEN do:
        ...
        FIND FIRST  bactivite NO-LOCK
            ...
            NO-ERROR.
        IF AVAILABLE(bactivite) THEN DO:
            RUN gAfficheMessageTemporaire("Activité","Il ne peut pas y avoir 2 débuts de journée dans la même journée !",FALSE,10,"OK","",FALSE,OUTPUT cRetour).
            RETURN.
        END.
        ...
        FIND FIRST  bactivite NO-LOCK
            ...
            NO-ERROR.
        IF AVAILABLE(bactivite) THEN DO:
            RUN gAfficheMessageTemporaire("Activité","Attention, il existe des activités saisies avant le début de journée. %sVoulez vous les supprimer ?",TRUE,15,"NON","",FALSE,OUTPUT cRetourDebut).
    ...
    IF cType-in = "FDJ" THEN do:
        ...
        FIND FIRST  bactivite NO-LOCK
            ...
            NO-ERROR.
        IF AVAILABLE(bactivite) THEN DO:
            RUN gAfficheMessageTemporaire("Activité","Il ne peut pas y avoir 2 fin de journée dans la même journée !",FALSE,10,"OK","",FALSE,OUTPUT cRetour).
            RETURN.
        END.
        ...
        IF dDate-in = ? THEN DO:
            FIND FIRST  bactivite NO-LOCK
                ...
                NO-ERROR.
            IF AVAILABLE(bactivite) THEN DO:
                RUN gAfficheMessageTemporaire("Activité","Attention, il existe des activités saisies après la fin de journée. %sVoulez vous les supprimer ?",TRUE,15,"NON","",FALSE,OUTPUT cRetourFin).
                /*IF cRetour = "NON" THEN RETURN.*/
            END.
        END.
    ...
    IF cType-in = "PAUSE" THEN do:
        ...
        IF AVAILABLE(bactivite) AND bactivite.cTypeActivite = "PAUSE" THEN DO:
            RUN gAfficheMessageTemporaire("Activité","Vous êtes déjà en activité de type 'PAUSE' !",FALSE,10,"OK","",FALSE,OUTPUT cRetour).
            RETURN.
        END.
        cCode = cType-in.
        cSousCode = cType-in.
    ...
    IF cType-in = "REPAS" THEN do:
        FIND LAST   bactivite NO-LOCK
            ...
            NO-ERROR.
        IF AVAILABLE(bactivite) THEN DO:
            RUN gAfficheMessageTemporaire("Activité","Vous avez déjà une activité de type 'REPAS' !",FALSE,10,"OK","",FALSE,OUTPUT cRetour).
            RETURN.
        END.
        cCode = cType-in.
        cSousCode = cType-in.
...
------------------------------------------------------------------------------*/
    ...
    dDateDeControle = DATE(gDonnePreference("DERNIERE_CONNEXION")).
    cMessage = "".
    FOR EACH    cActivite   NO-LOCK
        ...
        END.
    END.
    gSauvePreference("DERNIERE_CONNEXION",STRING(TODAY,"99/99/9999")).

    lREchercheDescendante = (gDonnePreference("PREFS_AFAIRE_RECHERCHE_DESCENDANTE") = "oui").

    IF cMessage <> "" THEN DO:
        cMessage = cMessage + CHR(10) + "Merci de contrôler ces jours pour vérifier si les activités sont correctes.". 
        /*RUN gAfficheMessageTemporaire("Activité",cMessage,FALSE,30,"OK","",FALSE,OUTPUT cRetour).*/
...
------------------------------------------------------------------------------*/
    ...
    IF cTypeRapport = "P" THEN DO:
        IF dDebut-in = ? THEN DO:
            RUN gAfficheMessageTemporaire("Activité","Merci de saisir une date de début",FALSE,10,"OK","",FALSE,OUTPUT cRetour).
            RETURN.
        END.
        IF dFin-in = ? THEN DO:
            RUN gAfficheMessageTemporaire("Activité","Merci de saisir une date de fin",FALSE,10,"OK","",FALSE,OUTPUT cRetour).
            RETURN.
        END.
    END.

    IF dFin-in < dDebut-in THEN DO:
        RUN gAfficheMessageTemporaire("Activité","Incohérence dans les dates",FALSE,5,"OK","",FALSE,OUTPUT cRetour).
        RETURN.
    END.


    cTitreEdition = "Activité de " + gDonneVraiNomUtilisateur(gcutilisateur)
    ...
    END.
    ELSE DO:
        RUN gAfficheMessageTemporaire("Activité","Aucune activité pour " + cGenreEtType,FALSE,5,"OK","",FALSE,OUTPUT cRetour).
    END.
...
------------------------------------------------------------------------------*/
    ...
    END.
    ELSE DO:
        RUN gAfficheMessageTemporaire("Activité","La recherche n'a donné aucun résultat",FALSE,5,"OK","",FALSE,OUTPUT cRetour).
    END.
...
DEFINE OUTPUT PARAMETER lRetour-ou AS LOGICAL NO-UNDO INIT TRUE.
    ...
    RUN gAfficheMessageTemporaire("Activité","Confirmez-vous la suppression de la ligne d'activité ?",TRUE,10,"NON","",FALSE,OUTPUT cRetour).
    IF cRetour = "NON"  THEN RETURN.

    /* Prévenir en cas de suppression de la ligne DDJ */
    IF ttActivites.cTypeActivite = "DDJ" THEN DO:
        RUN gAfficheMessageTemporaire("Activité","Vous êtes en train de supprimer la ligne de début de journée ! %sConfirmez-vous ?",TRUE,10,"NON","",FALSE,OUTPUT cRetour).
        IF cRetour = "NON" THEN RETURN.
    END.
    ...
    DO TRANS:
        ...
        IF NOT(AVAILABLE(activite)) THEN do:
            RUN gAfficheMessageTemporaire("Activité","Ligne d'activité introuvable !",FALSE,5,"OK","",FALSE,OUTPUT cRetour).
            RETURN.
        END.
        DELETE activite.
    END.
...
DEFINE OUTPUT PARAMETER lRetour-ou AS LOGICAL NO-UNDO INIT TRUE.
    ...
    IF cErreur <> "" THEN DO:
        cErreur = "Erreur lors de la saisie : " + replace(cErreur,"%s",CHR(10))
            .
        RUN gAfficheMessageTemporaire("Activité",cErreur,FALSE,15,"OK","",FALSE,OUTPUT cRetour).
        lRetour-ou = FALSE.
        RETURN.
    END.
    ...
    IF cAvertissement <> "" THEN DO:
        cAvertissement = "Avertissement : " + replace(cAvertissement,"%s",CHR(10)) 
            ...
            .
        RUN gAfficheMessageTemporaire("Activité",cAvertissement,FALSE,15,"OK","ACTIVITE-VALIDATION-INFO",FALSE,OUTPUT cRetour).
    END.
    ...
    DO TRANS:
        ...
        FIND FIRST activite WHERE Activite.idActivite = ttActivites.idActivite EXCLUSIVE-LOCK NO-ERROR.
        IF NOT(AVAILABLE(activite)) THEN do:
            RUN gAfficheMessageTemporaire("Activité","Ligne d'activité introuvable !",FALSE,0,"OK","",FALSE,OUTPUT cRetour).
            RETURN.
        END.
