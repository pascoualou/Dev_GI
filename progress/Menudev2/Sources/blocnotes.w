...
------------------------------------------------------------------------------*/
    ...
    DO WITH FRAME frmModule:
        ...
        gSauvePreference("NOTES-DERNIERE",cFichierNotes).
...
DEFINE OUTPUT PARAMETER lRetour-ou AS LOGICAL NO-UNDO.
    ...
    /* Rechargement de la liste */
    lCreation = TRUE.
    gSauvePreference("NOTES-DERNIERE",cFichierTempo).
    RUN Initialisation.
...
DEFINE VARIABLE cUtilisateurNote AS CHARACTER NO-UNDO.
    ...
    RUN gDechargeFichierEnLocal(cUtilisateurNote,cTempo).
    RUN gImpressionFichier(gcFichierLocal,ttNotes.cIdentFichier).
...
------------------------------------------------------------------------------*/
    ...
    cFichierNotes = gDonnePreference("NOTES-DERNIERE").
...
------------------------------------------------------------------------------*/
    ...
    IF available(ttnotes) THEN DO: 
        ...
        gmlog("Note en cours = " + ttNotes.cIdentFichier).
    ...
    DO WITH FRAME frmModule:
        REPEAT:
            ...
            gmlog("lTrouve = " + string(lTrouve)).
            /* Si trouvé : on sort */
            IF lTrouve THEN LEAVE.

            /* Si pas trouver on passe à l'onglet suivant */
            gmlog("avant find : AVAILABLE(ttNotes) = " + string(AVAILABLE(ttNotes))).
            IF cSens-in = "PREC" THEN DO:
            ...
            END.
            gmlog("apres find : AVAILABLE(ttNotes) = " + string(AVAILABLE(ttNotes))).
            ...
            IF AVAILABLE(ttNotes) THEN DO:
                ...
                gmlog("ttnotes.cIdentFichier = " + ttnotes.cIdentFichier + " - cOngletInitial = " + cOngletInitial).
