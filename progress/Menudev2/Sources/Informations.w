...
DO:
  ...
  IF ctempo = "" THEN DO:
    RUN gAfficheMessageTemporaire("Messages","Votre message est vide !",FALSE,0,"OK","",FALSE,OUTPUT cRetour).
    RETURN.
  END.
  ...
  IF NOT(AVAILABLE(ttutil)) THEN DO:
    RUN gAfficheMessageTemporaire("Messages","Veuillez sélectionner au moins un utilisateur !",FALSE,0,"OK","",FALSE,OUTPUT cRetour).
    RETURN.
  END.

  FOR EACH ttutil WHERE ttutil.lselection:
    RUN gEnvoiOrdre("INFOS",cTempo,ttutil.cutilisateur,gcUtilisateur,toggle-2:Checked,lMessageCollegue).
  END.
        ...
        FOR EACH ordres NO-LOCK
           ...
           :
            ...
            ASSIGN
                ...
                ttMessages.cVraiQui = gDonneVraiNomUtilisateur(ttMessages.cQui)
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH utilisateurs NO-LOCK
        ...
        CREATE ttutil.
        ASSIGN
            ttUtil.cVraiNom = gDonneVraiNomUtilisateur(utilisateur.cutilisateur)
    ...
    DO iBoucle = 1 TO NUM-ENTRIES(cOrdre-in):
        ...
        CASE cAction:
            WHEN "AFFICHE" THEN DO:
                IF gGetEtSupParam("INFOS-RECHARGER") = "OUI" THEN RUN Recharger.
...
------------------------------------------------------------------------------*/
    ...
    IF (gDonnePreference("INFOS-JOURS-MAX") = "" OR gDonnePreference("INFOS-JOURS-MAX") = ?) THEN DO:
        gSauvePreference("INFOS-JOURS-MAX","7").
    END.
...
------------------------------------------------------------------------------*/
    ...
    lDernierMessage = (gDonnePreference("PREF-MESSAGES-DERNIER") = "OUI").

    DO WITH FRAME frmModule:
        filInfosListe:SCREEN-VALUE = "(Par défaut, seuls les messages des " + gDonnePreference("INFOS-JOURS-MAX") + " derniers jours sont listés. Double clic sur le message pour répondre directement à l'utilisateur)".
    END.
...
------------------------------------------------------------------------------*/
    ...
    lDecroissant = (gDonnePreference("PREF-MESSAGES-DECROISSANT") = "OUI").
