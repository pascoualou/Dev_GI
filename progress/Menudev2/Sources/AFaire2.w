...
DO:
    IF AVAILABLE(ttActions) THEN DO:
        ...
        gSauvePreference("AFAIRE-ACTION",STRING(ttActions.iNumeroAction)).
        {&OPEN-QUERY-brwFichiers}
    END.
    ELSE DO:
...
DO:
    IF AVAILABLE(ttListes) THEN DO:
        ...
        gSauvePreference("AFAIRE-LISTE",string(ttListes.iNumeroListe)).
...
DO:
  ...
  /* Ajout d'une action dans la base */
    ...
    IF ENTRY(4,gcAllerRetour,"|") <> "" THEN DO:
        ...
        AFaire_Action.cFiller1 = GenereInfos("Création le %date% à %heure%").
    ...
    gSauvePreference("AFAIRE-ACTION",STRING(AFaire_Action.iNumeroAction)).
    RUN ChargeActions.
    RUN RaffListes.
    RUN MajHistorique.
...
DO:
    ...
    IF ttactions.iOrdreAction >= IMaxAction THEN RETURN.
        ...
        gSauvePreference("AFAIRE-ACTION",STRING(AFaire_Lien.iNumeroAction)).
        ...
        IF lCreerLeFichier THEN DO:
            ...
            */
            ...
            IF lErreurWord THEN DO:
                RUN gAfficheMessageTemporaire("Bases - Gestion commentaire",cErreurWord,FALSE,5,"OK","MESSAGE-COMMENTAIRE-WORD-1",FALSE,OUTPUT cRetour).
            END.
        END.
...
DO:
    ...
    gSauvePreference("AFAIRE-ACTION","0").
    RUN chargeActions.
    RUN GereActions.
    RUN raffListes.
...
DO:
  ...
  /* Ajout d'une liste dans la base */
    ...
    END.
    /* Rechargement de la liste */
    gSauvePreference("AFAIRE-LISTE",STRING(AFaire_Liste.iNumeroListe)).
    RELEASE AFaire_Liste.
    RUN ChargeListes.
END.
...
DO:
    ...
    cRepertoireProjet = gDonnePreference("PROJETS_REPERTOIRE_PERE").
...
DO:
  ...
  IF ttActions.cEtatAction = "PA" THEN cLibelle = "Penser à".
    ...
    IF AVAILABLE(AFaire_Action) THEN DO:
        ...
        AFaire_Action.cFiller1 = AFaire_Action.cFiller1 + "§" + cLibelle + GenereInfos(" le %date% à %heure%").
    END.
    ...
    AND gDonnePreference("PREF-ACTION-DEBUT-FIN") = "OUI"
    THEN DO: 
        FIND FIRST      AFaire_Lien     EXCLUSIVE-LOCK
                WHERE   AFaire_Lien.cUtilisateur = gcUtilisateur
                AND     AFaire_Lien.iNumeroListe = ttListes.iNumeroListe
    ...
    IF ttActions.cEtatAction = "F" AND gDonnePreference("PREF-ACTION-RETOUR-DEBUT") = "OUI" THEN DO:
        gSauvePreference("AFAIRE-ACTION",STRING(0)).
    END.
...
DO:
    gSauvePreference("PREF-AFAIRE-FICHIERS-TOUS-ACTION",SELF:SCREEN-VALUE).
    {&OPEN-QUERY-brwFichiers}
    RUN gereActions.
END.
...
DO:
  gSauvePreference("PREF-NONTERMINEES",(IF SELF:CHECKED THEN "OUI" ELSE "NON")).
  {&OPEN-QUERY-brwListes}
...
------------------------------------------------------------------------------*/
    ...
    DO WITH FRAME frmFonction:
        ...
        iActionEnCours = INTEGER(gDonnePreference("AFAIRE-ACTION")).
...
------------------------------------------------------------------------------*/
    ...
    IF gDonnePreference("PREF-AFAIRE-CONVERSION-FAITE") <> "OUI" THEN DO:
    ...
    DO WITH FRAME frmFonction:
        ...
        iListeEnCours = INTEGER(gDonnePreference("AFAIRE-LISTE")).
        FIND FIRST  ttListes 
...
------------------------------------------------------------------------------*/
    ...
    IF ERROR-STATUS:ERROR THEN DO:
        gSauvePreference("AFAIRE-LISTE","0").
        iListeEnCours = 0.
    END.
    ...
    cValeurTousAction = gDonnePreference("PREF-AFAIRE-FICHIERS-TOUS-ACTION").
    IF cValeurTousAction = "" THEN cValeurTousAction = "T".

    DO WITH FRAME frmFonction:
        brwActions:LOAD-MOUSE-POINTER(gcRepertoireRessources + "\curmenu.cur").
        tglNonTerminees:CHECKED = (IF gDonnePreference("PREF-NONTERMINEES") = "OUI" THEN TRUE ELSE FALSE).
...
------------------------------------------------------------------------------*/
    ...
    DEFINE VARIABLE cTempo AS CHARACTER NO-UNDO.
        ...
        IF gDonnePreference("PREF-ACTION-COMMENTAIRE-NOM") = "OUI" THEN DO:
        ...
        END.
    cRetour = gDonnePreference("PREF-REPERTOIRE-COMMENTAIRES") + "\C_" + cTempo + ".doc".
