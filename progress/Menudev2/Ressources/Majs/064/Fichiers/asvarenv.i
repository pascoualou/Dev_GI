...
prowin = "".
if search(dlc + "\bin\prowin32.exe") <> ? then DO:
    prowin = "prowin32.exe".
    Version_OE = "10".
    Suffixe = "".
END.
if search(dlc + "\bin\prowin.exe") <> ? then DO:
    prowin = "prowin.exe".
    Version_OE = "11".
    Suffixe = "-11".
END.
...
PROCEDURE GenerePF:
    ...
    PUT UNFORMATTED "# Fichier .pf généré par asvarenv.i pour menudev2 à partir de " + cFichierPfOriginal-in SKIP(1).
    
    REPEAT:
        IMPORT UNFORMATTED cLigne.
        
        /* La ligne est-elle à conserver */
        lAGarder = FALSE.
