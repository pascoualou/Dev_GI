...
rctBoutons-4 btnQuitter rctRaff rctRaff-2 filBoutonDev filBoutonPrec ~
...
btnCliSpe btnCliSuiv btnAssistance btnInternet btnGIDEV btnModifier ~
btnEmprunt btnPutty btnTeamviewer btnAbandon btnAjouter btnImprimer btnRaf ~
...
DEFINE MENU POPUP-MENU-btnGIDEV 
...
DEFINE BUTTON btnGIDEV  NO-FOCUS FLAT-BUTTON
     LABEL "Gi" 
...
ASSIGN 
       btnGIDEV:POPUP-MENU IN FRAME frmFond       = MENU POPUP-MENU-btnGIDEV:HANDLE.
...
DO:
    ...
    cCommande = DonnePreference("NAVIGATEUR-2").
...
DO:
  DEFINE VARIABLE cParametre AS CHARACTER NO-UNDO init "-".
  DEFINE VARIABLE lContinuer AS LOGICAL NO-UNDO INIT TRUE.

  /* Maj des versions */
  IF not(DonnePreference("AIDE-BOUTON-MANUELLE") = "OUI") THEN DO:
    RUN GereBoutons.
    RUN MessageStructure("CLI",OUTPUT lContinuer).
    IF NOT(lContinuer) THEN RETURN.
  END.

  IF DonnePreference("BATCH-CLIENT") = "" THEN RETURN.

  /* Récupération d'un éventuel parametre */
  IF DonnePreference("PREF-MAGICSCLI") = "OUI" THEN cParametre = DonnePreference("PREF-MAGIRESEAUCLI").

  cParametre = cParametre + " " + DonneRepGIApplication("CLI") + " " + DonneDisqueApplication("CLI").
    
  OS-COMMAND NO-WAIT VALUE("cmd /c """ + DonnePreference("BATCH-CLIENT") + """ " + cParametre).
END.
...
DO:
    ...
    IF DonnePreference("BATCH-CLIENT-PREC") = "" THEN RETURN.

    /* Récupération d'un éventuel parametre */
    IF DonnePreference("PREF-MAGICSPREC") = "OUI" THEN cParametre = DonnePreference("PREF-MAGIRESEAUPREC").

        cParametre = cParametre + " " + DonneRepGIApplication("PREC") + " " + DonneDisqueApplication("PREC").

        OS-COMMAND NO-WAIT VALUE("cmd /c """ + DonnePreference("BATCH-CLIENT-PREC") + """ " + cParametre).
...
DO:
    ...
    IF DonnePreference("BATCH-CLIENT-SPE") = "" THEN RETURN.

    /* Récupération d'un éventuel parametre */
    IF DonnePreference("PREF-MAGICSSPE") = "OUI" THEN cParametre = DonnePreference("PREF-MAGIRESEAUSPE").

        cParametre = cParametre + " " + DonneRepGIApplication("SPE") + " " + DonneDisqueApplication("SPE").

    OS-COMMAND NO-WAIT VALUE("cmd /c """ + DonnePreference("BATCH-CLIENT-SPE") + """ " + cParametre).
END.
...
DO:
    ...
    IF DonnePreference("BATCH-CLIENT-SUIV") = "" THEN RETURN.

    /* Récupération d'un éventuel parametre */
    IF DonnePreference("PREF-MAGICSSUIV") = "OUI" THEN cParametre = DonnePreference("PREF-MAGIRESEAUSUIV").

        cParametre = cParametre + " " + DonneRepGIApplication("SUIV") + " " + DonneDisqueApplication("SUIV").

    OS-COMMAND NO-WAIT VALUE("cmd /c """ + DonnePreference("BATCH-CLIENT-SUIV") + """ " + cParametre).
END.
...
&Scoped-define SELF-NAME btnGIDEV
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btnGIDEV winaide
ON CHOOSE OF btnGIDEV IN FRAME frmFond /* Gi */
DO:
    ...
    /* Au passage : Maj des versions */
    RUN GereBoutons.

        cBatch = DonnePreference("BATCH-DEV").
    IF cBatch = "" THEN RETURN.

    /* Récupération d'un éventuel parametre */
    /* - Travailler en mode client/serveur */
    IF DonnePreference("PREF-MAGICSDEV") = "OUI" THEN cParametre = Reseau. 
    
    cParametre = cParametre + " " + DonneRepGIApplication("DEV") + " " + DonneDisqueApplication("DEV").

    cCommande = "cmd /C """ + DonnePreference("BATCH-DEV") + """ " + cParametre.
    mlog ("cCommande = " + cCommande).
    OS-COMMAND NO-WAIT VALUE(cCommande).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btnGIDEV winaide
ON RIGHT-MOUSE-CLICK OF btnGIDEV IN FRAME frmFond /* Gi */
DO:
    RUN DonnePositionMessage IN ghGeneral.
    gcAllerRetour = STRING(giPosXMessage)
        + "|" + STRING(giPosYMessage)
        + "|" + "Type d'application (ADB/PME)"
        + "|" + DonnePreference("TYPE-APPLICATION").
    RUN VALUE(gcRepertoireExecution + "saisie2.w") (INPUT-OUTPUT gcAllerRetour).
    IF gcAllerRetour = "" THEN RETURN.
    SauvePreference("TYPE-APPLICATION",ENTRY(4,gcAllerRetour,"|")).

    cTypeApplication = DonnePreference("TYPE-APPLICATION").
    btnGIDEV:TOOLTIP = "Application GI (Développement) / Clic droit pour paramètrer le type d'application (actuellement : " + cTypeApplication + ")".
...
DO:
    ...
    cCommande = DonnePreference("NAVIGATEUR-1").
    IF cCommande = "" THEN RETURN.
    IF cCommande MATCHES "*.bat*" THEN
        OS-COMMAND NO-WAIT VALUE(gcRepertoireRessourcesPrivees + "\scripts\general\execute2.bat " + cCommande).
    ELSE
...
DO:
    ...
    gcAllerRetour = STRING(giPosXMessage)
        + "|" + STRING(giPosYMessage)
        + "|" + "Navigateur Internet"
        + "|" + DonnePreference("NAVIGATEUR").
    RUN VALUE(gcRepertoireExecution + "saisie2.w") (INPUT-OUTPUT gcAllerRetour).
    IF gcAllerRetour = "" THEN RETURN.
    SauvePreference("NAVIGATEUR",ENTRY(4,gcAllerRetour,"|")).
...
DO:
    ...
    cCommande = DonnePreference("BTNPERSO-1").
    IF cCommande = "" THEN RETURN.
    /*OS-COMMAND NO-WAIT VALUE(gcRepertoireRessourcesPrivees + "\scripts\general\execute.bat " + DonnePreference("BTNPERSO")).*/
    IF cCommande MATCHES "*.bat*" THEN
        OS-COMMAND NO-WAIT VALUE(gcRepertoireRessourcesPrivees + "\scripts\general\execute2.bat " + DonnePreference("BTNPERSO-1")).
    ELSE
        OS-COMMAND NO-WAIT VALUE(cCommande).
END.
...
DO:
    ...
    cCommande = DonnePreference("BTNPERSO-2").
    IF cCommande = "" THEN RETURN.
    /*OS-COMMAND NO-WAIT VALUE(gcRepertoireRessourcesPrivees + "\scripts\general\execute.bat " + DonnePreference("BTNPERSO")).*/
    IF cCommande MATCHES "*.bat*" THEN
        OS-COMMAND NO-WAIT VALUE(gcRepertoireRessourcesPrivees + "\scripts\general\execute2.bat " + DonnePreference("BTNPERSO-2")).
    ELSE
        OS-COMMAND NO-WAIT VALUE(cCommande).
END.
...
DO:
    ...
    cCommande = DonnePreference("BTNPERSO-3").
    IF cCommande = "" THEN RETURN.
    /*OS-COMMAND NO-WAIT VALUE(gcRepertoireRessourcesPrivees + "\scripts\general\execute.bat " + DonnePreference("BTNPERSO")).*/
    IF cCommande MATCHES "*.bat*" THEN
        OS-COMMAND NO-WAIT VALUE(gcRepertoireRessourcesPrivees + "\scripts\general\execute2.bat " + DonnePreference("BTNPERSO-3")).
    ELSE
        OS-COMMAND NO-WAIT VALUE(cCommande).
END.
...
DO:
    ...
    cCommande = DonnePreference("BTNPERSO-4").
    IF cCommande = "" THEN RETURN.
    /*OS-COMMAND NO-WAIT VALUE(gcRepertoireRessourcesPrivees + "\scripts\general\execute.bat " + DonnePreference("BTNPERSO")).*/
    IF cCommande MATCHES "*.bat*" THEN
        OS-COMMAND NO-WAIT VALUE(gcRepertoireRessourcesPrivees + "\scripts\general\execute2.bat " + DonnePreference("BTNPERSO-4")).
    ELSE
        OS-COMMAND NO-WAIT VALUE(cCommande).
END.
...
DO:
    ...
    gcAllerRetour = STRING(giPosXMessage)
        + "|" + STRING(giPosYMessage)
        + "|" + "Bouton Perso"
        + "|" + DonnePreference("BTNPERSO-4").
    RUN VALUE(gcRepertoireExecution + "saisie2.w") (INPUT-OUTPUT gcAllerRetour).
    IF gcAllerRetour = "" THEN RETURN.
    SauvePreference("BTNPERSO-4",ENTRY(4,gcAllerRetour,"|")).
...
DO:
    ...
    cCommande = DonnePreference("BTNPERSO-5").
    IF cCommande = "" THEN RETURN.
    /*OS-COMMAND NO-WAIT VALUE(gcRepertoireRessourcesPrivees + "\scripts\general\execute.bat " + DonnePreference("BTNPERSO")).*/
    IF cCommande MATCHES "*.bat*" THEN
        OS-COMMAND NO-WAIT VALUE(gcRepertoireRessourcesPrivees + "\scripts\general\execute2.bat " + DonnePreference("BTNPERSO-5")).
    ELSE
        OS-COMMAND NO-WAIT VALUE(cCommande).
END.
...
DO:
    ...
    cCommande = DonnePreference("BTNPERSO-6").
    IF cCommande = "" THEN RETURN.
    /*OS-COMMAND NO-WAIT VALUE(gcRepertoireRessourcesPrivees + "\scripts\general\execute.bat " + DonnePreference("BTNPERSO")).*/
    IF cCommande MATCHES "*.bat*" THEN
        OS-COMMAND NO-WAIT VALUE(gcRepertoireRessourcesPrivees + "\scripts\general\execute2.bat " + DonnePreference("BTNPERSO-6")).
    ELSE
        OS-COMMAND NO-WAIT VALUE(cCommande).
END.
...
DO:
    IF DonnePreference("PUTTY") = "" THEN RETURN.
    OS-COMMAND NO-WAIT VALUE(gcRepertoireRessourcesPrivees + "\scripts\general\execute.bat " + DonnePreference("PUTTY")).
END.
...
DO:
    ...
    gcAllerRetour = STRING(giPosXMessage)
        + "|" + STRING(giPosYMessage)
        + "|" + "Putty"
        + "|" + DonnePreference("PUTTY").
    RUN VALUE(gcRepertoireExecution + "saisie2.w") (INPUT-OUTPUT gcAllerRetour).
    IF gcAllerRetour = "" THEN RETURN.
    SauvePreference("PUTTY",ENTRY(4,gcAllerRetour,"|")).
     RUN GereTooltipPerso.  
END.
...
DO:
    ...
    lMaj = (DonneParametre("FORCAGE-MAJ") = "TRUE").

    IF DonnePreference("PREF-POSITION") = "OUI" THEN DO:
        cTempo1 = STRING(WinAide:X) + "|" + STRING(WinAide:Y).
        SauvePreference("POSITION",cTempo1).
    END.
    ELSE
        SauvePreference("POSITION","").
    
    IF NOT(lMaj) THEN DO:
        IF DonnePreference("PREF-SORTIE") = "OUI" THEN DO:
            RUN AfficheMessageAvecTemporisation("Confirmation...","Confirmez-vous la sortie de menudev ?",TRUE,10,"OUI","",FALSE,OUTPUT cRetour).
...
DO:
    IF DonnePreference("REPERTOIRE-CLIENT-PFGI") = "" THEN RETURN.
    /*MESSAGE Loc_appli + "\exe\giserver.exe " + DonnePreference("REPERTOIRE-CLIENT-PFGI") VIEW-AS ALERT-BOX.*/
    OS-COMMAND NO-WAIT VALUE(Loc_appli + "\exe\giserver.exe " + DonnePreference("REPERTOIRE-CLIENT-PFGI")).
...
DO:
    ...
    gcAllerRetour = STRING(giPosXMessage)
        ...
        + "|" + DonnePreference("REPERTOIRE-CLIENT-PFGI").
    RUN VALUE(gcRepertoireExecution + "saisie2.w") (INPUT-OUTPUT gcAllerRetour).
    IF gcAllerRetour = "" THEN RETURN.
    SauvePreference("REPERTOIRE-CLIENT-PFGI",ENTRY(4,gcAllerRetour,"|")).
     RUN GereTooltipPerso.  
END.
...
DO:
    IF DonnePreference("TEAMVIEWER") = "" THEN RETURN.
    OS-COMMAND NO-WAIT VALUE(gcRepertoireRessourcesPrivees + "\scripts\general\execute.bat " + DonnePreference("TEAMVIEWER")).
END.
...
DO:
    ...
    gcAllerRetour = STRING(giPosXMessage)
        + "|" + STRING(giPosYMessage)
        + "|" + "Team Viewer"
        + "|" + DonnePreference("TEAMVIEWER").
    RUN VALUE(gcRepertoireExecution + "saisie2.w") (INPUT-OUTPUT gcAllerRetour).
    IF gcAllerRetour = "" THEN RETURN.
    SauvePreference("TEAMVIEWER",ENTRY(4,gcAllerRetour,"|")).
     RUN GereTooltipPerso.  
END.
...
------------------------------------------------------------------------------*/
  ...
  IF DonnePreference("PREF-DEMON_TICKETS") = "OUI" THEN DO:
    RUN TicketAuto.
  END.
  
  IF DonnePreference("PREF-SAISIEMASTERGI") = "OUI" THEN DO:
      cTitre = "Login".
  ...
  IF DonnePreference("PREF-SAISIEINTERNET") = "OUI" THEN DO:
      ...
      IF hFenetre <> 0 THEN DO : 
          IF itemporisation4 >= giLatenceInternet THEN DO:
            /*MESSAGE "Passe = " DonnePreference("PREF-PASSEINTERNET") VIEW-AS ALERT-BOX.*/
              /*OS-COMMAND SILENT VALUE(loc_outils + "\OuvertureInternet.exe """ + cTitre + """ " + DonnePreference("PREF-PASSEINTERNET") ).*/
              OS-COMMAND SILENT VALUE(loc_outils + "\SaisieAutomatique.exe """ + cTitre + """ """ + DonnePreference("PREF-PASSEINTERNETUTIL") + "%tab%" + DonnePreference("PREF-PASSEINTERNET") + "%enter%%ctrl%t""" ).
            itemporisation4 = 0.
          END.
...
DO:
    SauvePreference("AIDE-BOUTON-DEV",SELF:SCREEN-VALUE).
    RUN GereTooltipPerso.
END.
...
DO:
    SauvePreference("AIDE-BOUTON-CLI",SELF:SCREEN-VALUE).
    RUN GereTooltipPerso.
END.
...
DO:
    SauvePreference("AIDE-BOUTON-PREC",SELF:SCREEN-VALUE).
    RUN GereTooltipPerso.
END.
...
DO:
    ...
    SauvePreference("AIDE-BOUTON-SPE",SELF:SCREEN-VALUE).
    RUN GereTooltipPerso.
END.
...
DO:
    ...
    SauvePreference("AIDE-BOUTON-SUIV",SELF:SCREEN-VALUE).
    RUN GereTooltipPerso.
END.
...
DO:
    ...
    SauvePreference("PREF-COULEUR-MODULE-" + ttModulesUtilisateur.cIdent,STRING(ttModulesUtilisateur.icouleur)).
...
DO:
    ...
    SauvePreference("PREF-VISIBLE-MODULE-" + ttModulesUtilisateur.cIdent,"NON").
...
DO:
    ...
    gcAllerRetour = STRING(giPosXMessage)
        + "|" + STRING(giPosYMessage)
        + "|" + "Nom du Bouton Perso"
        + "|" + DonnePreference("BTNPERSO-1-NOM").
    RUN VALUE(gcRepertoireExecution + "saisie2.w") (INPUT-OUTPUT gcAllerRetour).
    IF gcAllerRetour = "" THEN RETURN.
    SauvePreference("BTNPERSO-1-NOM",ENTRY(4,gcAllerRetour,"|")).
    RUN GereTooltipPerso.    
END.
...
DO:
    ...
    gcAllerRetour = STRING(giPosXMessage)
        + "|" + STRING(giPosYMessage)
        + "|" + "Nom du Bouton Perso"
        + "|" + DonnePreference("BTNPERSO-2-NOM").
    RUN VALUE(gcRepertoireExecution + "saisie2.w") (INPUT-OUTPUT gcAllerRetour).
    IF gcAllerRetour = "" THEN RETURN.
    SauvePreference("BTNPERSO-2-NOM",ENTRY(4,gcAllerRetour,"|")).
    RUN GereTooltipPerso.    
END.
...
DO:
    ...
    gcAllerRetour = STRING(giPosXMessage)
        + "|" + STRING(giPosYMessage)
        + "|" + "Nom du Bouton Perso"
        + "|" + DonnePreference("BTNPERSO-3-NOM").
    RUN VALUE(gcRepertoireExecution + "saisie2.w") (INPUT-OUTPUT gcAllerRetour).
    IF gcAllerRetour = "" THEN RETURN.
    SauvePreference("BTNPERSO-3-NOM",ENTRY(4,gcAllerRetour,"|")).
    RUN GereTooltipPerso.    
END.
...
DO:
    ...
    gcAllerRetour = STRING(giPosXMessage)
        + "|" + STRING(giPosYMessage)
        + "|" + "Nom du Bouton Perso"
        + "|" + DonnePreference("BTNPERSO-4-NOM").
    RUN VALUE(gcRepertoireExecution + "saisie2.w") (INPUT-OUTPUT gcAllerRetour).
    IF gcAllerRetour = "" THEN RETURN.
    SauvePreference("BTNPERSO-4-NOM",ENTRY(4,gcAllerRetour,"|")).
    RUN GereTooltipPerso.    
END.
...
DO:
    ...
    gcAllerRetour = STRING(giPosXMessage)
        + "|" + STRING(giPosYMessage)
        + "|" + "Nom du Bouton Perso"
        + "|" + DonnePreference("BTNPERSO-5-NOM").
    RUN VALUE(gcRepertoireExecution + "saisie2.w") (INPUT-OUTPUT gcAllerRetour).
    IF gcAllerRetour = "" THEN RETURN.
    SauvePreference("BTNPERSO-5-NOM",ENTRY(4,gcAllerRetour,"|")).
    RUN GereTooltipPerso.    
END.
...
DO:
    ...
    gcAllerRetour = STRING(giPosXMessage)
        + "|" + STRING(giPosYMessage)
        + "|" + "Nom du Bouton Perso"
        + "|" + DonnePreference("BTNPERSO-6-NOM").
    RUN VALUE(gcRepertoireExecution + "saisie2.w") (INPUT-OUTPUT gcAllerRetour).
    IF gcAllerRetour = "" THEN RETURN.
    SauvePreference("BTNPERSO-6-NOM",ENTRY(4,gcAllerRetour,"|")).
    RUN GereTooltipPerso.    
END.
...
DO:
    ...
    gcAllerRetour = STRING(giPosXMessage)
        + "|" + STRING(giPosYMessage)
        + "|" + "Bouton Perso"
        + "|" + DonnePreference("BTNPERSO-1").
    RUN VALUE(gcRepertoireExecution + "saisie2.w") (INPUT-OUTPUT gcAllerRetour).
    IF gcAllerRetour = "" THEN RETURN.
    SauvePreference("BTNPERSO-1",ENTRY(4,gcAllerRetour,"|")).
...
DO:
    ...
    gcAllerRetour = STRING(giPosXMessage)
        + "|" + STRING(giPosYMessage)
        + "|" + "Bouton Perso"
        + "|" + DonnePreference("BTNPERSO-2").
    RUN VALUE(gcRepertoireExecution + "saisie2.w") (INPUT-OUTPUT gcAllerRetour).
    IF gcAllerRetour = "" THEN RETURN.
    SauvePreference("BTNPERSO-2",ENTRY(4,gcAllerRetour,"|")).
...
DO:
    ...
    gcAllerRetour = STRING(giPosXMessage)
        + "|" + STRING(giPosYMessage)
        + "|" + "Bouton Perso"
        + "|" + DonnePreference("BTNPERSO-3").
    RUN VALUE(gcRepertoireExecution + "saisie2.w") (INPUT-OUTPUT gcAllerRetour).
    IF gcAllerRetour = "" THEN RETURN.
    SauvePreference("BTNPERSO-3",ENTRY(4,gcAllerRetour,"|")).
...
DO:
    ...
    gcAllerRetour = STRING(giPosXMessage)
        + "|" + STRING(giPosYMessage)
        + "|" + "Bouton Perso"
        + "|" + DonnePreference("BTNPERSO-5").
    RUN VALUE(gcRepertoireExecution + "saisie2.w") (INPUT-OUTPUT gcAllerRetour).
    IF gcAllerRetour = "" THEN RETURN.
    SauvePreference("BTNPERSO-5",ENTRY(4,gcAllerRetour,"|")).
...
DO:
    ...
    gcAllerRetour = STRING(giPosXMessage)
        + "|" + STRING(giPosYMessage)
        + "|" + "Bouton Perso"
        + "|" + DonnePreference("BTNPERSO-6").
    RUN VALUE(gcRepertoireExecution + "saisie2.w") (INPUT-OUTPUT gcAllerRetour).
    IF gcAllerRetour = "" THEN RETURN.
    SauvePreference("BTNPERSO-6",ENTRY(4,gcAllerRetour,"|")).
...
DO:
    ...
    gcAllerRetour = STRING(giPosXMessage)
        ...
        + "|" + DonnePreference("NAVIGATEUR-1").
    RUN VALUE(gcRepertoireExecution + "saisie2.w") (INPUT-OUTPUT gcAllerRetour).
    IF gcAllerRetour = "" THEN RETURN.
    SauvePreference("NAVIGATEUR-1",ENTRY(4,gcAllerRetour,"|")).
    RUN GereTooltipPerso.  
END.
...
DO:
    ...
    gcAllerRetour = STRING(giPosXMessage)
        ...
        + "|" + DonnePreference("NAVIGATEUR-2").
    RUN VALUE(gcRepertoireExecution + "saisie2.w") (INPUT-OUTPUT gcAllerRetour).
    IF gcAllerRetour = "" THEN RETURN.
    SauvePreference("NAVIGATEUR-2",ENTRY(4,gcAllerRetour,"|")).
    RUN GereTooltipPerso.  
END.
...
DO:
    ...
    cTypeApplication = DonnePreference("TYPE-APPLICATION").
    btnGIDEV:TOOLTIP IN FRAME frmFond = "Application GI (Développement) / Clic droit pour paramètrer le type d'application (actuellement : " + cTypeApplication + ")".
END.
...
DO:
    ...
    gcAllerRetour = STRING(giPosXMessage)
        ...
        + "|" + DonnePreference("BATCH-CLIENT").
    RUN VALUE(gcRepertoireExecution + "saisie2.w") (INPUT-OUTPUT gcAllerRetour).
    IF gcAllerRetour = "" THEN RETURN.
    SauvePreference("BATCH-CLIENT",ENTRY(4,gcAllerRetour,"|")).
     RUN GereTooltipPerso.
     RUN GereBoutons.
END.
...
DO:
    ...
    gcAllerRetour = STRING(giPosXMessage)
        + "|" + STRING(giPosYMessage)
        + "|" + "Nom complet du batch de lancement"
        + "|" + DonnePreference("BATCH-CLIENT-PREC").
    RUN VALUE(gcRepertoireExecution + "saisie2.w") (INPUT-OUTPUT gcAllerRetour).
    IF gcAllerRetour = "" THEN RETURN.
    SauvePreference("BATCH-CLIENT-PREC",ENTRY(4,gcAllerRetour,"|")).
    RUN GereTooltipPerso.  
    RUN GereBoutons.
END.
...
DO:
    ...
    gcAllerRetour = STRING(giPosXMessage)
        + "|" + STRING(giPosYMessage)
        + "|" + "Nom complet du batch de lancement"
        + "|" + DonnePreference("BATCH-CLIENT-SPE").
    RUN VALUE(gcRepertoireExecution + "saisie2.w") (INPUT-OUTPUT gcAllerRetour).
    IF gcAllerRetour = "" THEN RETURN.
    SauvePreference("BATCH-CLIENT-SPE",ENTRY(4,gcAllerRetour,"|")).
     RUN GereTooltipPerso.  
     RUN GereBoutons.
END.
...
DO:
    ...
    gcAllerRetour = STRING(giPosXMessage)
        + "|" + STRING(giPosYMessage)
        + "|" + "Nom complet du batch de lancement"
        + "|" + DonnePreference("BATCH-CLIENT-SUIV").
    RUN VALUE(gcRepertoireExecution + "saisie2.w") (INPUT-OUTPUT gcAllerRetour).
    IF gcAllerRetour = "" THEN RETURN.
    SauvePreference("BATCH-CLIENT-SUIV",ENTRY(4,gcAllerRetour,"|")).
     RUN GereTooltipPerso.  
     RUN GereBoutons.
END.
...
DO:
  ...
  IF DonnePreference("PREF-COULEUR-MODULE-" + "Favoris") = "" THEN
      SauvePreference("PREF-COULEUR-MODULE-" + "Favoris","12").
  IF DonnePreference("PREF-COULEUR-MODULE-" + "RDev") = "" THEN
      SauvePreference("PREF-COULEUR-MODULE-" + "RDev","2").
  IF DonnePreference("PREF-COULEUR-MODULE-" + "ROut") = "" THEN
      SauvePreference("PREF-COULEUR-MODULE-" + "ROut","9").

   FOR EACH ttModulesUtilisateur:
       ttModulesUtilisateur.iCouleur = INTEGER(DonnePreference("PREF-COULEUR-MODULE-" + ttModulesUtilisateur.cIdent)).
   END.
...
DO:
    ...
    SauvePreference("PREF-BROUILLON-X","0").
    SauvePreference("PREF-BROUILLON-Y","0").
END.
...
*/
    ...
    WinAide:Y = 0.
    IF DonnePreference("PREF-POSITION") = "OUI" THEN DO:
        cTempo1 = DonnePreference("POSITION").
    ...
    RUN enable_UI.
    RUN Initialisation.
    IF not(DonnePreference("PREF-MODESILENCIEUX")) = "OUI" THEN do:
...
------------------------------------------------------------------------------*/
    ...
    IF ((WEEKDAY(TODAY) = 1 OR WEEKDAY(TODAY) = 7) AND DonnePreference("PREFS-ABSENCES-PAS-AVERTISSEMENT-WE") = "OUI") THEN
        RETURN.

    /* Affichage des absences si demandé */
    ASSIGN dTempo = date(DonnePreference("PREF-ABSENCES-PREVENU")) NO-ERROR.
    ...
    IF dTempo = ? OR dTempo < TODAY THEN DO:
        ...
        IF dTempo <> ? AND dTempo <> DATE(1,1,2000) AND dTempo < TODAY THEN DO:
            SupprimePreference("ABSENCES-SIGNALEES-*").
        END.
        RUN DonneAbsences(FALSE,OUTPUT cAbsJour, OUTPUT cAbsFutures).
        ...
        SauvePreference("PREF-ABSENCES-PREVENU",STRING(TODAY)).
        IF cTempo <> "" THEN DO:
...
------------------------------------------------------------------------------*/
    ...
    OR gcDroitsUtilisateur matches "*simulation_version*" THEN DO:
        /*
        RUN AfficheMessageAvecTemporisation("Informations","Le passage de la version "
...
------------------------------------------------------------------------------*/
    ...
    IF SEARCH(gcRepertoireRessourcesPrivees + "Majs\" + STRING(giVersionUtilisateur + 1,"999") + "\maj.bat") <> ? THEN DO: 
        RUN AfficheMessageAvecTemporisation("Informations","Nouvelle version de menudev2 disponible : "
                                        + string(giVersionUtilisateur + 1,"999")
        ...
        AssigneParametre("FORCAGE-MAJ","TRUE").

        /* Pour activer l'onglet A propos à la réouverture */
        /*SauvePreference("DERNIER-MODULE","APropos").*/
  ...
  IF cSens-in <> "init" THEN DO:
       ...
       FOR EACH bttModulesUtilisateur
           ...
           ELSE DO:
               iTempoOrdre = iTempoOrdre + 10.
               bttModulesUtilisateur.iOrdre = iTempoOrdre.
               SauvePreference("PREF-ORDRE-MODULE-" + bttModulesUtilisateur.cIdent, STRING(bttModulesUtilisateur.iOrdre)).
           END.
      END.
   END.
   ELSE DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE DonnePositionMenudev winaide 
...
------------------------------------------------------------------------------*/
  ...
  ENABLE rctBoutons rctBoutons-3 rctBoutons-5 rctBoutons-4 btnQuitter rctRaff 
         rctRaff-2 filBoutonDev filBoutonPrec filBoutongi filBoutonSuiv 
         ...
         btnCliSuiv btnAssistance btnInternet btnGIDEV btnModifier btnEmprunt 
...
------------------------------------------------------------------------------*/
    ...
    cRepertoireVersion = disque + DonneRepertoireApplication(cVersion-in).

    IF SEARCH(cRepertoireVersion + "\nul") = ? THEN DO:
        cTempo = "Répertoire demandé '" + cRepertoireVersion + "' inexistant".
        RUN AfficheMessageAvecTemporisation("Informations",cTempo,FALSE,10,"OK","",FALSE,OUTPUT cRetour).
    END.
    ELSE DO:
...
------------------------------------------------------------------------------*/
    DO WITH FRAME frmFond:
        ...
        MENU m_Gestion_de_la_version_PREC:SENSITIVE = (DonnePreference("BATCH-CLIENT-PREC") MATCHES ("*Scripts\session\Lancement.bat*")).
        MENU m_Gestion_de_la_version_CLI:SENSITIVE = (DonnePreference("BATCH-CLIENT") MATCHES ("*Scripts\session\Lancement.bat*")).
        MENU m_Gestion_de_la_version_SUIV:SENSITIVE = (DonnePreference("BATCH-CLIENT-SUIV") MATCHES ("*Scripts\session\Lancement.bat*")).
...
------------------------------------------------------------------------------*/
    ...
    IF cAction-in = "EFFACE-ORDRE-TOUS" THEN DO:
        RUN AfficheMessageAvecTemporisation("Confirmation","Confirmez-vous la suppression de TOUS LES MESSAGES ?",TRUE,10,"NON","MESSAGE-SUPPRESSION-1",FALSE,OUTPUT cRetour).
        IF cRetour = "NON" THEN RETURN.
    ...
    IF cAction-in = "REPONSE" THEN DO:
        ...
        OR cTempo = "Neptune2"
        THEN DO:
            RUN AfficheMessageAvecTemporisation("Informations","Impossible de répondre à cet utilisateur",FALSE,5,"OK","MESSAGE-IMPOSSIBLE",FALSE,OUTPUT cRetour).
            RETURN.
        END.
        ...
        RUN EnvoiOrdre("INFOS",ENTRY(4,gcAllerRetour,"|"),cTempo,gcUtilisateur,false,TRUE).

    END.

    AssigneParametre("INFOS-RECHARGER","OUI").
...
------------------------------------------------------------------------------*/
    ...
    lVersionManuelle = (DonnePreference("AIDE-BOUTON-MANUELLE") = "OUI").
    
    /* Tooltip du bouton perso */
    DO WITH FRAME frmFond :
        IF DonnePreference("BTNPERSO-1-NOM") <> "" THEN
            btnPerso-1:TOOLTIP = DonnePreference("BTNPERSO-1-NOM") + " / Clic droit pour paramètrer".
        ELSE IF DonnePreference("BTNPERSO-1") <> "" THEN
            btnPerso-1:TOOLTIP = DonnePreference("BTNPERSO-1") + " / Clic droit pour paramètrer".
        ELSE    
        btnPerso-1:TOOLTIP = "Bouton perso. / Clic droit pour paramètrer".

        IF DonnePreference("BTNPERSO-2-NOM") <> "" THEN
            btnPerso-2:TOOLTIP = DonnePreference("BTNPERSO-2-NOM") + " / Clic droit pour paramètrer".
        ELSE IF DonnePreference("BTNPERSO-2") <> "" THEN
            btnPerso-2:TOOLTIP = DonnePreference("BTNPERSO-2") + " / Clic droit pour paramètrer".
        ELSE    
            btnPerso-2:TOOLTIP = "Bouton perso. / Clic droit pour paramètrer".

        IF DonnePreference("BTNPERSO-3-NOM") <> "" THEN
            btnPerso-3:TOOLTIP = DonnePreference("BTNPERSO-3-NOM") + " / Clic droit pour paramètrer".
        ELSE IF DonnePreference("BTNPERSO-3") <> "" THEN
            btnPerso-3:TOOLTIP = DonnePreference("BTNPERSO-3") + " / Clic droit pour paramètrer".
        ELSE    
            btnPerso-3:TOOLTIP = "Bouton perso. / Clic droit pour paramètrer".

        IF DonnePreference("BTNPERSO-4-NOM") <> "" THEN
            btnPerso-4:TOOLTIP = DonnePreference("BTNPERSO-4-NOM") + " / Clic droit pour paramètrer".
        ELSE IF DonnePreference("BTNPERSO-4") <> "" THEN
            btnPerso-4:TOOLTIP = DonnePreference("BTNPERSO-4") + " / Clic droit pour paramètrer".
        ELSE    
            btnPerso-4:TOOLTIP = "Bouton perso. / Clic droit pour paramètrer".

        IF DonnePreference("BTNPERSO-5-NOM") <> "" THEN
            btnPerso-5:TOOLTIP = DonnePreference("BTNPERSO-5-NOM") + " / Clic droit pour paramètrer".
        ELSE IF DonnePreference("BTNPERSO-5") <> "" THEN
            btnPerso-5:TOOLTIP = DonnePreference("BTNPERSO-5") + " / Clic droit pour paramètrer".
        ELSE    
            btnPerso-5:TOOLTIP = "Bouton perso. / Clic droit pour paramètrer".

        IF DonnePreference("BTNPERSO-6-NOM") <> "" THEN
            btnPerso-6:TOOLTIP = DonnePreference("BTNPERSO-6-NOM") + " / Clic droit pour paramètrer".
        ELSE IF DonnePreference("BTNPERSO-6") <> "" THEN
            btnPerso-6:TOOLTIP = DonnePreference("BTNPERSO-6") + " / Clic droit pour paramètrer".
        ELSE    
            btnPerso-6:TOOLTIP = "Bouton perso. / Clic droit pour paramètrer".

        IF DonnePreference("BATCH-CLIENT") <> "" THEN
            btnCli:TOOLTIP = DonnePreference("BATCH-CLIENT") + " / Clic droit pour paramètrer".
        ELSE    
            btnCli:TOOLTIP = "Bouton GI (Cli) / Clic droit pour paramètrer".
        
        IF DonnePreference("BATCH-CLIENT-SUIV") <> "" THEN
            btnCliSuiv:TOOLTIP = DonnePreference("BATCH-CLIENT-SUIV") + " / Clic droit pour paramètrer".
        ELSE    
            btnCliSuiv:TOOLTIP = "Bouton GI (Suiv) / Clic droit pour paramètrer".
        
        IF DonnePreference("BATCH-CLIENT-PREC") <> "" THEN
            btnCliPrec:TOOLTIP = DonnePreference("BATCH-CLIENT-PREC") + " / Clic droit pour paramètrer".
        ELSE    
            btnCliPrec:TOOLTIP = "Bouton GI (Prec)/ Clic droit pour paramètrer".

        IF DonnePreference("BATCH-CLIENT-SPE") <> "" THEN
            btnCliSpe:TOOLTIP = DonnePreference("BATCH-CLIENT-SPE") + " / Clic droit pour paramètrer".
        ELSE    
            btnCliSpe:TOOLTIP = "Bouton GI (Spe)/ Clic droit pour paramètrer".
        

        IF DonnePreference("NAVIGATEUR") <> "" THEN
            btnInternet:TOOLTIP = DonnePreference("NAVIGATEUR") + " / Clic droit pour paramètrer".
        ELSE    
            btnInternet:TOOLTIP = "Bouton Internet / Clic droit pour paramètrer".

        IF DonnePreference("ASSISTANCE") <> "" THEN
            btnAssistance:TOOLTIP = DonnePreference("ASSISTANCE") + " / Clic droit pour paramètrer".
        ELSE    
            btnAssistance:TOOLTIP = "Bouton Assistance. / Clic droit pour paramètrer".

        IF DonnePreference("PUTTY") <> "" THEN
            btnPutty:TOOLTIP = DonnePreference("PUTTY") + " / Clic droit pour paramètrer".
        ELSE    
            btnPutty:TOOLTIP = "Bouton Putty / Clic droit pour paramètrer".

        IF DonnePreference("TEAMVIEWER") <> "" THEN
            btnTeamviewer:TOOLTIP = DonnePreference("TEAMVIEWER") + " / Clic droit pour paramètrer".
        ELSE    
            btnTeamviewer:TOOLTIP = "Bouton TeamViewer / Clic droit pour paramètrer".

        IF DonnePreference("REPERTOIRE-CLIENT-PFGI") <> "" THEN
            btnServeurs-2:TOOLTIP = DonnePreference("REPERTOIRE-CLIENT-PFGI") + " / Clic droit pour paramètrer".
        ELSE    
...
------------------------------------------------------------------------------*/
    ...
    IF NOT lReinitialisation-in THEN DO:
        ...
        END.  
        SauvePreference("PREF-BOUTON-PERSO-" + cNumeroBouton-in + "-IMAGE",cFichierImageSansRepertoire).
        cFichierACharger = cRepertoireImages + cFichierImageSansRepertoire.
    END.
    ELSE DO:
        IF DonnePreference("PREF-BOUTON-PERSO-" + cNumeroBouton-in + "-IMAGE-DEFAUT") = "" THEN DO:
            cFichierImageSansRepertoire = cNumeroBouton-in + ".ico".
            SauvePreference("PREF-BOUTON-PERSO-" + cNumeroBouton-in + "-IMAGE","").
            cFichierACharger = gcRepertoireImages + cFichierImageSansRepertoire.
        END.
        ELSE DO:
            cFichierACharger = gcRepertoireImages + DonnePreference("PREF-BOUTON-PERSO-" + cNumeroBouton-in + "-IMAGE-DEFAUT").
        END.
    END.
       ...
       :
        ...
        ELSE DO:
            ...
            cLibelleMessage = "Le " + string(ordres.ddate,"99/99/9999") + " à " + STRING(ordres.iordre,"hh:mm") 
               + " (" + DonneVraiNomUtilisateur(ordres.filler) + ")"
               + " > " + ordres.cmessage.
            ...
            IF ordres.lPrioritaire THEN DO:
                lImportant = TRUE.
                RUN AfficheMessageAvecTemporisation("!!! MESSAGE IMPORTANT !!!",cLibelleMessage2,FALSE,0,"OK","",TRUE,OUTPUT cRetour).
            END.
    
            cLibelleBulle = cLibelleBulle 
                          + (IF cLibelleBulle <> "" THEN "%s" ELSE "")
                          + "(" + DonneVraiNomUtilisateur(ordres.filler) + ")" + " > " + ordres.cmessage.
...
------------------------------------------------------------------------------*/
    ...
    /* Gestion de l'utilisateur */
    RUN GereUtilisateur("ENTREE").
    IF glUtilTrace THEN mLogDebug("GereUtilisateur : Fin",ETIME).

    /* Log de l'application */
    glLogActif = (DonnePreference("PREF-ACTIVELOG") = "OUI").
    giLatenceMax = INTEGER(DonnePreference("LATENCE-MASTERGI")).
    giLatenceInternet = INTEGER(DonnePreference("LATENCE-INTERNET")).
    glDeveloppeur = (DonnePreference("PREF-DEVELOPPEUR") = "OUI").
    ...
    RUN ChargeDefinitions.
    IF glUtilTrace THEN mLogDebug("ChargeDefinitions : Fin",ETIME).
    ...
    IF DonnePreference("PREF-EXECUTIONBATCHENTREE") = "OUI" THEN DO:
    ...
    DO WITH FRAME frmfond:
        ...
        btnGIDEV:LOAD-IMAGE(gcRepertoireImages + "gi.ico").
        btnGIDEV:LOAD-IMAGE-INSENSITIVE(gcRepertoireImages + "gi-off.ico").
    ...
    FOR EACH gttModules 
        ...
        IF not(gttModules.lVisible) AND not(DonnePreference("PREF-VOIRMODULESINVISIBLES") = "OUI")  THEN NEXT.
        ...
        ttModulesUtilisateur.lInvisible = (DonnePreference("PREF-VISIBLE-MODULE-" + gttModules.cIdent) = "NON").
        ttModulesUtilisateur.iCouleur = INTEGER(DonnePreference("PREF-COULEUR-MODULE-" + gttModules.cIdent)).
        ...
        END.
        ELSE DO:
            iOrdreModule = integer(DonnePreference("PREF-ORDRE-MODULE-" + gttModules.cIdent)).
    ...
    FOR EACH ttModulesUtilisateur
        ...
        :
        SauvePreference("PREF-ORDRE-MODULE-" + ttModulesUtilisateur.cIdent,STRING(ttModulesUtilisateur.iOrdre)).
        SauvePreference("PREF-VISIBLE-MODULE-" + ttModulesUtilisateur.cIdent,"OUI").
        SauvePreference("PREF-COULEUR-MODULE-" + ttModulesUtilisateur.cIdent,STRING(ttModulesUtilisateur.iCouleur)).
    END.
    ...
    cTempo1 = DonnePreference("DERNIER-MODULE").
    IF cTempo1 <> "" THEN do:
        FIND FIRST ttModulesUtilisateur
    ...
    Mlog("#TITRE#Synchronisation du timer sur la minute exacte :"
    ...
    OS-COMMAND NO-WAIT VALUE("net share Bases=" + DonnePreference("REPERTOIRE-BASES")).
    ...
    IF DonnePreference("PREF-BROUILLON") = "OUI" THEN RUN LanceBrouillon.
...
------------------------------------------------------------------------------*/
    ...
    RUN EcritLogAgenda("Lancement du brouillon").
...
------------------------------------------------------------------------------*/
    ...
    IF lGeneral-in OR DonnePreference("PREFS-MODULE-DIRECT") <> "OUI" THEN DO:
        RUN AffichageFrames("Prefs").
    END.
    ELSE DO:
        /* appel des preferences du module si disponible */
        cProgPrefs = gcRepertoireExecution + "prefs-" + DonneProgramme(gcModuleEnCours) + ".w".
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
   MENU-ITEM m_Patch_Suiv:SENSITIVE IN MENU POPUP-MENU-btnCliSuiv = NOT(DonnePreference("PREF-MAGICSSUIV") = "OUI").
   MENU-ITEM m_Patch_Prec:SENSITIVE IN MENU POPUP-MENU-btnCliPrec = NOT(DonnePreference("PREF-MAGICSPREC") = "OUI").
   MENU-ITEM m_Patch_Cli:SENSITIVE IN MENU POPUP-MENU-btnCli = NOT(DonnePreference("PREF-MAGICSCLI") = "OUI").
   MENU-ITEM m_Patch_Spe:SENSITIVE IN MENU POPUP-MENU-btnCliSpe = NOT(DonnePreference("PREF-MAGICSSPE") = "OUI").
...
------------------------------------------------------------------------------*/
    ...
    IF DonneEtSupprimeParametre("CHANGEMENT-STRUCTURE-" + cVersion-in) = "OUI" THEN DO:
        RUN AfficheMessageAvecTemporisation("Informations","ATTENTION : La version de la base a changé pour cette version '" + cVersion-in + "'.%sPensez à monter la version de vos bases concernées.%sVoulez-vous continuer ?",TRUE,10,"OUI","MESSAGE-CHANGEMENT-STRUCTURE",FALSE,OUTPUT cRetour).
...
------------------------------------------------------------------------------*/
    ...
    DEFINE BUFFER bOrdres FOR ordres.
     ...
     IF DonnePreference("PREF-MESSAGES-DERNIER-INCORPORATION") = "OUI" THEN DO:
...
------------------------------------------------------------------------------*/
    ...
    /* Récupération d'un éventuel parametre */
    /* - Travailler en mode client/serveur */
    IF DonnePreference("PREF-MAGICS" + cVersion-in) = "OUI" THEN cParametre = Reseau. 
    cParametre = cParametre + " " + DonneRepGIApplication(cVersion-in) + " " + DonneDisqueApplication(cVersion-in).

    OS-COMMAND value("%reseau%dev\outils\progress\Menudev2\Ressources\Scripts\session\Lancement.bat " + cVersion-in + " %reseau%dev\outils\progress\Menudev2\Ressources\Scripts\session\LancePatchGI.bat " + cParametre).
...
------------------------------------------------------------------------------*/
    ...
    OS-COMMAND value("%reseau%dev\outils\progress\Menudev2\Ressources\Scripts\session\Lancement.bat " + cVersion-in + " %reseau%dev\outils\progress\Menudev2\Ressources\Scripts\session\LanceRepMacros.bat " + cParametre).
END PROCEDURE.
...
------------------------------------------------------------------------------*/
    ...
    RUN FormateNomFichier(cPrefixe,"",".7z", OUTPUT cNomDefaut). 
    RUN DonnePositionMessage IN ghGeneral.
    gcAllerRetour = STRING(giPosXMessage)
        + "|" + STRING(giPosYMessage)
        + "|" + "Nom de la sauvegarde"
    ...
    cCommandeShell = gcRepertoireRessourcesPrivees + "\scripts\serveurs\_GI-SauvegardeVersion.bat " + cRepertoireVersion + " " + cNomDefaut + " -mx" + DonnePreference("PREF-COMPRESSION").
    RUN ExecuteCommandeDos(cCommandeShell).
...
------------------------------------------------------------------------------*/
    ...
    IF DonnePreference("PREF-DERNIER-MODULE") = "OUI" THEN SauvePreference("DERNIER-MODULE",gcModuleEnCours).

    /* Lancement du batch de fin */
    IF DonnePreference("PREF-EXECUTIONBATCHSORTIE") = "OUI" THEN DO:
...
define variable lVerbose                as logical      no-undo.
    ...
    RepertoireSurveillance = DonnePreference("PREF-DEMON_TICKETS-REPERTOIRE").
    lVerbose = (DonnePreference("PREF-DEMON_TICKETS-VERBOSE") = "OUI").
    ...
    FOR EACH ttFichiers:
        ...
        Mlog("TicketAuto - Traitement de : " + ttFichiers.cNomCompletFichier).
        ...
        Mlog("TicketAuto - Reference / Ticket : " + cReference + " / " + cTicket).
...
------------------------------------------------------------------------------*/
   ...
   FOR EACH gttModules
       WHERE valid-handle(gttModules.hModule):
       ETIME(TRUE).
       Mlog("TOP général - Début traitement : " + gttModules.cIdent).
       RUN DonneOrdre(gttModules.cIdent,"TOPGENERAL",OUTPUT lRetour).
       Mlog("TOP général - Fin traitement : " + gttModules.cIdent + " (" + STRING(ETIME) + "ms)").
   END.

   RUN ControleVersionDisponible.

   IF DonnePreference("PREF-EXECUTIONBATCHMINUTES") = "OUI" THEN 
   ...
   IF lAction5m THEN DO:
       IF DonnePreference("PREF-VERIFICATIONPARTAGE") = "OUI" THEN DO:
   ...
   IF giHeure = 0 THEN DO:
       ...
       AssigneParametre("ABSENCES-RECHARGER","OUI").
       AssigneParametre("ACCUEIL-RECHARGER","OUI").
       ...
       /*SupprimePreference("ABSENCES-SIGNALEES-*").*/ /* fait dans AfficheAbsences */
   END.

   IF ENTRY(2,cHeure,":") = "00" OR DonnePreference("PREF-ABSENCES-PREVENIR-NOUVELLE-DESUITE") = "OUI" THEN DO:
...
------------------------------------------------------------------------------*/
    ...
    cTempo = DonnePreference("PREF-BOUTON-PERSO-" + cNumeroBouton-in + "-IMAGE").
    IF cTempo = "" THEN do:
...
------------------------------------------------------------------------------*/
    ...
    IF DonnePreference("AIDE-BOUTON-MANUELLE") = "OUI" THEN DO:
        cRetour = DonnePreference("AIDE-BOUTON-" + cVersion-in).            
        Mlog("Aide boutons manuelle pour " + cVersion-in + " : cRetour = " + cRetour).
    END.
    ELSE DO:
        Mlog("Aide boutons automatique").
        /* Lancement du batch de sauvegarde */
        CASE cVersion-in:
            WHEN "PREC" OR WHEN "SUIV" OR WHEN "SPE" THEN DO:
                cRepertoireVersion = "gi_" + cVersion-in + "\" + "gi". 
        ...
        IF SEARCH(cFichierVersion) <> ? THEN DO:
            ...
            cVersionAvantControle = DonnePreference("AIDE-BOUTON-" + cVersion-in + "-SVG").
            ...
            IF NUM-ENTRIES(cVersionAvantControle,".") >= 2 AND NUM-ENTRIES(cVersionApresControle,".") >= 2 THEN DO:
                ...
                IF cVersionBaseAvantControle <> cVersionBaseApresControle THEN DO:
                    AssigneParametre("CHANGEMENT-STRUCTURE-" + cVersion-in,"OUI").
                END.
            END.
    
            /* Sauvegarde de la version actuelle */
            SauvePreference("AIDE-BOUTON-" + cVersion-in + "-SVG",cRetour).
        END.
    END.
