...
DO:
    ...
    RUN recharger.
    RUN AfficheMessageAvecTemporisation("Statistiques MaGI","Rechargement des statistiques terminé.",FALSE,5,"OK","MESSAGE-STATS-RECHARGEMENT",FALSE,OUTPUT cRetour).
END.
...
------------------------------------------------------------------------------*/
    ...
    cTempo2 = DonnePreference("TRAVAIL-STATS-CHARGEMENT-EN-COURS").
    IF cTempo2 <> "" THEN DO:
        cTempo = "Un chargement est déjà en cours par " + cTempo2 + ".".
        RUN AfficheMessageAvecTemporisation("Statistiques MaGI",cTempo,FALSE,5,"OK","",FALSE,OUTPUT cRetour).
        RETURN.
    END.

    /* Demande de confirmation */
    cTempo = "Confirmez-vous le (re)chargement des statistiques MaGI".
    RUN AfficheMessageAvecTemporisation("Statistiques MaGI",cTempo,TRUE,5,"NON","",FALSE,OUTPUT cRetour).
    IF cRetour = "NON" THEN RETURN.

    /* Avertir que le chargement est en cours */
    SauvePreference("TRAVAIL-STATS-CHARGEMENT-EN-COURS",gcUtilisateur).
    ...
    SauvePreference("TRAVAIL-STATS-DERNIER-CHARGEMENT","Le " + STRING(TODAY,"99/99/9999") + " à " + STRING(TIME,"HH:MM:SS")).
    ...
    DO iBoucle = 1 TO NUM-ENTRIES(cOrdre-in):
        ...
        CASE cAction:
            WHEN "AFFICHE" THEN DO:
                IF ModuleARecharger("bypass.txt") THEN RUN Recharger.
...
------------------------------------------------------------------------------*/
    ...
    gcAideModifier = "#" + (IF ModificationAutorisee("bypass.txt") THEN "DIRECT" ELSE "INTERDIT") + "#Modifier la liste des ByPass".
...
------------------------------------------------------------------------------*/
    ...
    /* Dernier chargement */
    filDernierChargement:SCREEN-VALUE = filDernierChargement:PRIVATE-DATA + " " + DonnePreference("TRAVAIL-STATS-DERNIER-CHARGEMENT").
