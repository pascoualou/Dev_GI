    ...
    DO iBoucle = 1 TO NUM-ENTRIES(cOrdre-in):
        ...
        CASE cAction:
            WHEN "AFFICHE" THEN DO:
                IF ModuleARecharger("menudev.inf") THEN RUN Recharger.
...
DEFINE INPUT PARAMETER hBouton AS HANDLE NO-UNDO.
    ...
    Mlog("iMaxCol = " + STRING(iMaxCol)).

    /* Recherche de la 1ere colonne disponible */
    iColonne = 0.
    DO iBoucle = 1 TO 4:
        iMaxColx = INTEGER(DonnePreference("MAXCOL" + STRING(iBoucle))).
        Mlog("iMaxCol" + STRING(iBoucle) + " = " + STRING(iMaxColx)).
...
/*    MESSAGE "Bouton ajouté aux favoris." VIEW-AS ALERT-BOX INFORMATION TITLE "Gestion des favoris".*/
    Mlog ("Fichier des favoris : " + gcRepertoireRessourcesPrivees + "Utilisateurs\" + gcUtilisateur + "\" + gcUtilisateur + "-Favoris.mdev2").
...
------------------------------------------------------------------------------*/
    ...
    RUN ExtraitFichierDeLaBase("","menudev.inf").
