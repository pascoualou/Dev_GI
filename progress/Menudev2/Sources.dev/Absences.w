...
DO:
    ...
    ELSE DO:
        ...
        END.
        ELSE DO:
            IF DonnePreference("PREFS-ABSENCES-PAS-WE") = "OUI" THEN DO:
  ...
  DEFINE INPUT PARAMETER cQuand-in AS CHARACTER NO-UNDO.
    ...
    /* si pas de selection, on ne fait rien */
        IF brwAbsences:NUM-SELECTED-ROWS < 1 THEN do:
            RUN AfficheMessageAvecTemporisation("Absences","Aucune ligne sélectionnée !",FALSE,0,"OK","",FALSE,OUTPUT cRetour).
            RETURN.
        END.
        ...
        DO iSelection = 1 TO brwAbsences:NUM-SELECTED-ROWS:          
            ...
            FOR EACH    utilisateurs    NO-LOCK
               :
                IF DonnePreferenceUtilisateur(utilisateur.cutilisateur,"PREF-ABSENCES-PREVENIR-NOUVELLE") = "OUI" THEN DO:
                    SauvePreferenceUtilisateur(utilisateur.cutilisateur,"PREF-ABSENCES-PREVENU","").
                END.
            END.
        END.
    ...
    DO iBoucle = 1 TO NUM-ENTRIES(cOrdre-in):
        ...
        CASE cAction:
            WHEN "AFFICHE" THEN DO:
                ...
                IF DonneEtSupprimeParametre("ABSENCES-RECHARGER") = "OUI" THEN DO:
                    RUN Recharger.
                END.
...
------------------------------------------------------------------------------*/
    ...
    iVersionMenudev2 = DonneVersionMenudev2().
    ...
    ttUtils.cVraiNom = DonneVraiNomUtilisateur(gcUtilisateur).
    ...
    REPEAT:
        ...
        IF (DonnePreference("PREFS-ABSENCES-PAS-WE") = "OUI" AND (WEEKDAY(dDateEnCours) = 7 OR WEEKDAY(dDateEnCours) = 1)) THEN lJourAPrendre = FALSE.
        
        IF lJourAPrendre THEN DO:

            CREATE ttAbsences.
            ttAbsences.ddate = dDateEnCours.
            ttAbsences.cJour = RemplaceVariables("%j% %jj% %m%","*",dDateEnCours,0).
            ttAbsences.iSemaine = DonneSemaine(dDateEnCours).
            ttAbsences.lFerie = JourFerie(dDateEnCours).
            ttAbsences.lScolaire = VacancesScolaires(dDateEnCours).
...
------------------------------------------------------------------------------*/
    ...
    /*IF NOT(JourFerie(dDate-in)) THEN DO:*/
        FIND FIRST  ttUtils
            WHERE   ttUtils.iColonne = iNumero-in
            NO-ERROR.
        IF AVAILABLE(ttUtils) THEN DO:
