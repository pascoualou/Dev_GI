    ...
    DO iBoucle = 1 TO NUM-ENTRIES(cOrdre-in):
        ...
        CASE cAction:
            WHEN "AFFICHE" THEN DO:
                IF ModuleARecharger("passes.txt") THEN RUN Recharger.
...
------------------------------------------------------------------------------*/
    ...
    gcAideModifier = "#" + (IF ModificationAutorisee("passes.txt") THEN "DIRECT" ELSE "INTERDIT") + "#Modifier la liste des mots de passe".
...
------------------------------------------------------------------------------*/
    ...
    DEFINE VARIABLE cFichierBP AS CHARACTER NO-UNDO.
        ...
        RUN ExtraitFichierDeLaBase("","passes.txt").
...
DEFINE OUTPUT PARAMETER lRetour-ou AS LOGICAL NO-UNDO.
    ...
    AssigneParametre("FICHIERS-INFOSFICHIER","passes.txt" + ",PARAM").
