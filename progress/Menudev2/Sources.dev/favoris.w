...
DEFINE INPUT PARAMETER hBouton AS HANDLE NO-UNDO.
    ...
    cCommande = RemplaceVariables(cCommande,"*",?,0).
...
DEFINE INPUT PARAMETER hBouton AS HANDLE NO-UNDO.
    ...
    iMaxCol = INTEGER(DonnePreference("MAXCOL")).
    ...
    iMaxColx = INTEGER(DonnePreference("MAXCOL" + STRING(iColonneNouvelle))).
...
------------------------------------------------------------------------------*/
    ...
    lBoutonsPlats = (IF DonnePreference("PREF-BOUTONSNORMAUX") = "OUI" THEN FALSE ELSE TRUE).
    ...
    Mlog("Recherche du fichier des favoris : " + cFichierFavoris).
    IF SEARCH(cFichierFavoris) = ? THEN do:
        Mlog ("Fichier des favoris inexistant").
        /*RETURN.*/
    END.
    ELSE DO:
        ...
        SauvePreference("MAXCOL1",string(irow[1])).
        SauvePreference("MAXCOL2",string(irow[2])).
        SauvePreference("MAXCOL3",string(irow[3])).
        SauvePreference("MAXCOL4",string(irow[4])).
        SauvePreference("MAXCOL","7").
